...
class MakeGraph:
    ...
    def graph_edge(self, from_node, to_node, options=None):
        if not options:
            options = {}
        for option, value in self.edge_options(from_node,
                                               to_node).iteritems():
            if option not in options:
                options[option] = value
        self.debug_print("Edge: %s -> %s (%s)" % (from_node, to_node,
                                                  repr(options)))
    ...
    def task_files(self, directory, file_pattern='*tasks.yaml'):
        if os.path.isfile(directory) and fnmatch.fnmatch(directory,
                                                         file_pattern):
            yield directory
...
parser.add_argument("--group", "-g",
                    help="Group or stage to build the graph for",
                    default=None)
