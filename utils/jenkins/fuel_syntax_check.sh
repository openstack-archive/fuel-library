...
function check_lint {
  ...
  if [ -f Gemfile ] && grep -q puppet-lint Gemfile ; then
    bundle install
...
for mod in $modules; do
  echo -e "\nChecking $mod"
  pushd $mod &> /dev/null
  check_lint || let fail+=1
  check_syntax || let fail+=1
  popd &>/dev/null
done

if test $fail -ne 0 ; then
  echo "Syntax Test FAILED: $fail modules failed syntax tests."
  exit 1
else
  echo "Syntax Test SUCCEEDED: All modules successfully passed syntax tests."
  exit 0
fi
