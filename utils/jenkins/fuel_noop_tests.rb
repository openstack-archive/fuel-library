...
module NoopTests
  ...
  def self.puppet_tasks_without_specs
    ...
    modular_puppet_task_files.each do |task|
      ...
      tasks << task unless File.exist? spec_path(spec, false)
    end
    tasks
  end

  # append absolute or relative path prefix
  # to a spec name
  # @param [String] spec Spec name
  # @param [TrueClass,FalseClass] relative Relative or absolute prefix?
  # @return [String]
  def self.spec_path(spec, relative=true)
    if relative
