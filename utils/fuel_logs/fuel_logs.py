    ...
    def output(self):
        ...
        for record in self.log:
            ...
            if not (log and time and line):
                continue
            IO.output("%s %s %s\n" % (self.node_name(log), time.isoformat(), line))

    def clear(self):
        self.log = []

    def sort_log(self):
        self.log = sorted(self.log, key=lambda record: record.get('time', None))
...
def options():
    ...
    parser.add_argument("--sort",  "-s", action="store_true", default=False, help='Sort Puppet logs by date')
    parser.add_argument("--evals",  "-e", action="store_true", default=False, help='Show Puppet evaltrace lines')
    parser.add_argument("--less",  "-l", action="store_true", default=False, help='Redirect data to the "less" pager')
