...
class OpenStackSwiftCompactCase(OpenStackSwiftCompactSitePPBaseTestCase):
    def test_deploy_open_stack_swift_compact(self):
        self.validate(
            ...
            'puppet agent --test --tags "compact_controller"')
        self.validate(
...
#            node.save_snapshot('openstack')
        ...
        for node in [self.controller1,self.controller2,self.controller3]:
            ...
            results.append(execute(remote.sudo.ssh, 'puppet agent --test --tags "role_swift_storage"'))
            results.append(execute(remote.sudo.ssh, 'puppet agent --test --tags "role_swift_storage"'))
        for node in [self.controller1,self.controller2,self.controller3]:
            remote = ssh(node.ip_address, username='root', password='r00tme')
            results.append(execute(remote.sudo.ssh, 'puppet agent --test --tags "role_swift_proxy"'))
        for node in [self.controller1,self.controller2,self.controller3]:
            remote = ssh(node.ip_address, username='root', password='r00tme')
            results.append(execute(remote.sudo.ssh, 'puppet agent --test --tags "role_swift_storage"'))
