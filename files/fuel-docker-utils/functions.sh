...
function get_docker_version {
  echo $(${DOCKER} -v | awk -F'/' '{ print $2 }')
}

# This function is used to check that a version is less than or equal to a
# specific version.
# Usage:
#   versioncmp_lte <version> <version to compare to>
# Example
#   versioncmp_lte 2.1.1 2.2.2 -> returns 0
#   versioncmp_lte 2.3.1 2.2.2 -> returns 1
function versioncmp_lte {
  [ "$1" = "$(echo -e "$1\n$2" | sort -V | head -n1)" ]
}
