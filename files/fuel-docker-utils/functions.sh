...
function show_usage {
  ...
  echo "  create: create container without run it"
  ...
  while [ -n "$1" ]; do
    case "$1" in
      ...
      --)           shift
                    ...
                    return
                    ;;
      help|create|build|start|check|list|copy|restart|stop|revert|shell|upgrade|restore|backup|destroy|logs|post_start_hooks)
                    nonopts+=("$@")
                    return
                    ;;
