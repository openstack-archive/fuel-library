...
%define predefined_modules  %{_sourcedir}/predefined_modules.tar.gz
...
%build
if test -s %{predefined_modules}; then
   tar xzvf  %{predefined_modules} -C %{_builddir}/%{name}-%{version}/deployment/puppet/
   else
