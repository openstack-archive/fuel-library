...
concat::fragment { 'nova-api-1_balancermember_nova-api-1':
  ...
  content => '  server node-1 10.108.2.4:8773  check
  server node-2 10.108.2.5:8773  check
  server node-3 10.108.2.6:8773  check
',
...
concat::fragment { 'nova-api-1_listen_block':
  content => '
listen nova-api-1
  bind 10.108.1.2:8773 ssl crt /var/lib/astute/haproxy/public_haproxy.pem
  bind 10.108.2.2:8773 
  timeout server  600s
',
...
concat::fragment { 'nova-api-2_balancermember_nova-api-2':
  ...
  content => '  server node-1 10.108.2.4:8774  check inter 10s fastinter 2s downinter 3s rise 3 fall 3
  server node-2 10.108.2.5:8774  check inter 10s fastinter 2s downinter 3s rise 3 fall 3
  server node-3 10.108.2.6:8774  check inter 10s fastinter 2s downinter 3s rise 3 fall 3
',
...
concat::fragment { 'nova-api-2_listen_block':
  content => '
listen nova-api-2
  bind 10.108.1.2:8774 ssl crt /var/lib/astute/haproxy/public_haproxy.pem
  bind 10.108.2.2:8774 
  option  httpchk
  option  httplog
  option  httpclose
  timeout server  600s
...
concat::fragment { 'nova-metadata-api_balancermember_nova-metadata-api':
  ...
  content => '  server node-1 10.108.2.4:8775  check inter 10s fastinter 2s downinter 3s rise 3 fall 3
  server node-2 10.108.2.5:8775  check inter 10s fastinter 2s downinter 3s rise 3 fall 3
  server node-3 10.108.2.6:8775  check inter 10s fastinter 2s downinter 3s rise 3 fall 3
',
...
concat::fragment { 'nova-novncproxy_balancermember_nova-novncproxy':
  ...
  content => '  server node-1 10.108.2.4:6080  check
  server node-2 10.108.2.5:6080  check
  server node-3 10.108.2.6:6080  check
',
...
listen nova-novncproxy
  bind 10.108.1.2:6080 ssl crt /var/lib/astute/haproxy/public_haproxy.pem
  balance  roundrobin
  option  httplog
',
...
haproxy::listen { 'nova-api-1':
  bind             => {'10.108.1.2:8773' => ['ssl', 'crt', '/var/lib/astute/haproxy/public_haproxy.pem'], '10.108.2.2:8773' => ''},
  bind_options     => '',
...
haproxy::listen { 'nova-api-2':
  bind             => {'10.108.1.2:8774' => ['ssl', 'crt', '/var/lib/astute/haproxy/public_haproxy.pem'], '10.108.2.2:8774' => ''},
  bind_options     => '',
  collect_exported => 'true',
  name             => 'nova-api-2',
  notify           => 'Exec[haproxy-restart]',
