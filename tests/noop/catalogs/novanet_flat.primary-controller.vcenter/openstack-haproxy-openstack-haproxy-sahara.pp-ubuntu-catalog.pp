...
concat::fragment { 'sahara_balancermember_sahara':
  ...
  content => '  server node-1 10.108.2.4:8386  check
  server node-2 10.108.2.5:8386  check
  server node-3 10.108.2.6:8386  check
',
...
concat::fragment { 'sahara_listen_block':
  content => '
listen sahara
  bind 10.108.1.2:8386 ssl crt /var/lib/astute/haproxy/public_haproxy.pem
  bind 10.108.2.2:8386 
  balance  roundrobin
  option  httplog
',
...
haproxy::listen { 'sahara':
  bind             => {'10.108.1.2:8386' => ['ssl', 'crt', '/var/lib/astute/haproxy/public_haproxy.pem'], '10.108.2.2:8386' => ''},
  bind_options     => '',
