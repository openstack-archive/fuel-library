...
ceph_conf { 'client.radosgw.gateway/host':
  before => 'Package[apache2]',
  name   => 'client.radosgw.gateway/host',
  value  => 'node-125',
}

ceph_conf { 'client.radosgw.gateway/keyring':
  before => 'Package[apache2]',
  name   => 'client.radosgw.gateway/keyring',
  value  => '/etc/ceph/keyring.radosgw.gateway',
}

ceph_conf { 'client.radosgw.gateway/rgw_data':
  before => 'Package[apache2]',
  name   => 'client.radosgw.gateway/rgw_data',
  value  => '/var/lib/ceph/radosgw',
}

ceph_conf { 'client.radosgw.gateway/rgw_dns_name':
  before => 'Package[apache2]',
  name   => 'client.radosgw.gateway/rgw_dns_name',
  value  => '*.pp',
}

ceph_conf { 'client.radosgw.gateway/rgw_keystone_accepted_roles':
  before => 'Package[apache2]',
  name   => 'client.radosgw.gateway/rgw_keystone_accepted_roles',
  value  => '_member_, Member, admin, swiftoperator',
}

ceph_conf { 'client.radosgw.gateway/rgw_keystone_admin_token':
  before => 'Package[apache2]',
  name   => 'client.radosgw.gateway/rgw_keystone_admin_token',
  value  => 'UxFQFw3m',
}

ceph_conf { 'client.radosgw.gateway/rgw_keystone_revocation_interval':
  before => 'Package[apache2]',
  name   => 'client.radosgw.gateway/rgw_keystone_revocation_interval',
  value  => '1000000',
}

ceph_conf { 'client.radosgw.gateway/rgw_keystone_token_cache_size':
  before => 'Package[apache2]',
  name   => 'client.radosgw.gateway/rgw_keystone_token_cache_size',
  value  => '10',
}

ceph_conf { 'client.radosgw.gateway/rgw_keystone_url':
  before => 'Package[apache2]',
  name   => 'client.radosgw.gateway/rgw_keystone_url',
  value  => '192.168.0.7:35357',
}

ceph_conf { 'client.radosgw.gateway/rgw_print_continue':
  before => 'Package[apache2]',
  name   => 'client.radosgw.gateway/rgw_print_continue',
  value  => 'true',
}

ceph_conf { 'client.radosgw.gateway/rgw_socket_path':
  before => 'Package[apache2]',
  name   => 'client.radosgw.gateway/rgw_socket_path',
  value  => '/tmp/radosgw.sock',
}

ceph_conf { 'client.radosgw.gateway/user':
  before => 'Package[apache2]',
  name   => 'client.radosgw.gateway/user',
  value  => 'www-data',
}
...
class { 'Ceph::Radosgw':
  ...
  rgw_keystone_url                 => '192.168.0.7:35357',
...
exec { 'Populate client.radosgw.gateway keyring':
  before  => 'File[/etc/ceph/keyring.radosgw.gateway]',
  command => 'ceph auth get-or-create client.radosgw.gateway > /etc/ceph/keyring.radosgw.gateway',
  creates => '/etc/ceph/keyring.radosgw.gateway',
  cwd     => '/root',
  path    => ['/bin/', '/sbin/', '/usr/bin/', '/usr/sbin/'],
}

exec { 'ceph create client.radosgw.gateway':
  before  => 'Exec[Populate client.radosgw.gateway keyring]',
  command => 'ceph auth get-or-create client.radosgw.gateway osd 'allow rwx' mon 'allow rw'',
  cwd     => '/root',
  path    => ['/bin/', '/sbin/', '/usr/bin/', '/usr/sbin/'],
}
...
file { '/etc/apache2/sites-available//fastcgi.conf':
  before  => 'Exec[ceph create client.radosgw.gateway]',
...
',
  ...
  mode    => '0755',
  owner   => 'www-data',
  path    => '/etc/apache2/sites-available//fastcgi.conf',
}

file { '/etc/apache2/sites-available//rgw.conf':
  before  => ['File[/etc/apache2/sites-enabled/rgw.conf]', 'Exec[ceph create client.radosgw.gateway]'],
  content => 'Listen :6780
...
file { '/etc/apache2/sites-enabled/rgw.conf':
  ...
  group  => 'www-data',
  notify => ['Service[httpd]', 'Service[httpd]'],
  owner  => 'www-data',
  path   => '/etc/apache2/sites-enabled/rgw.conf',
  target => '/etc/apache2/sites-available//rgw.conf',
}
...
file { '/etc/ceph/keyring.radosgw.gateway':
  before => 'Firewall[012 RadosGW allow]',
  group  => 'www-data',
  mode   => '0640',
  owner  => 'www-data',
...
file { '/var/www/radosgw/s3gw.fcgi':
  before  => 'Exec[ceph create client.radosgw.gateway]',
  content => '#!/bin/sh
...
',
  ...
  mode    => '0755',
  owner   => 'www-data',
  path    => '/var/www/radosgw/s3gw.fcgi',
}

file { '/var/www/radosgw':
  ensure  => 'directory',
  before  => 'Exec[ceph create client.radosgw.gateway]',
  group   => 'www-data',
  mode    => '0755',
  owner   => 'www-data',
  path    => '/var/www/radosgw',
  recurse => 'true',
}
...
file { 'fastcgi.load symlink':
  ...
  require => ['File[fastcgi.load]', 'Exec[mkdir /etc/apache2/mods-enabled]'],
...
file { 'fastcgi.load':
  ...
  before  => 'File[/etc/apache2/mods-available]',
  content => 'LoadModule fastcgi_module /usr/lib/apache2/modules/mod_fastcgi.so
',
  group   => 'root',
  mode    => '0644',
  notify  => 'Class[Apache::Service]',
  owner   => 'root',
  path    => '/etc/apache2/mods-available/fastcgi.load',
  require => ['Package[httpd]', 'Exec[mkdir /etc/apache2/mods-available]'],
}
...
file { 'rewrite.load symlink':
  ...
  require => ['File[rewrite.load]', 'Exec[mkdir /etc/apache2/mods-enabled]'],
...
keystone_endpoint { 'RegionOne/swift':
  ...
  admin_url    => 'http://192.168.0.7:8080/swift/v1',
  internal_url => 'http://192.168.0.7:8080/swift/v1',
  name         => 'RegionOne/swift',
  public_url   => 'https://172.16.0.3:8080/swift/v1',
}
...
package { 'httpd':
  ...
  before => ['Package[radosgw]', 'Package[libapache2-mod-fastcgi]', 'Package[libnss3-tools]'],
  name   => 'apache2',
  notify => 'Class[Apache::Service]',
}
...
package { 'libapache2-mod-fastcgi':
  ...
  before => ['File[/etc/apache2/sites-available//rgw.conf]', 'File[/etc/apache2/sites-available//rgw.conf]', 'File[/etc/apache2/sites-available//fastcgi.conf]', 'File[/var/www/radosgw/s3gw.fcgi]', 'File[/etc/apache2/ssl/]', 'File[/var/lib/ceph/radosgw/ceph-radosgw.gateway]', 'File[/var/lib/ceph/radosgw]', 'File[/var/www/radosgw]', 'File[/etc/ceph/nss]', 'File[/var/log/ceph/radosgw.log]'],
  name   => 'libapache2-mod-fastcgi',
}

package { 'libnss3-tools':
  ensure => 'installed',
  before => ['File[/etc/apache2/sites-available//rgw.conf]', 'File[/etc/apache2/sites-available//fastcgi.conf]', 'File[/var/www/radosgw/s3gw.fcgi]', 'File[/etc/apache2/ssl/]', 'File[/var/lib/ceph/radosgw/ceph-radosgw.gateway]', 'File[/var/lib/ceph/radosgw]', 'File[/var/www/radosgw]', 'File[/etc/ceph/nss]', 'File[/var/log/ceph/radosgw.log]'],
  name   => 'libnss3-tools',
}
