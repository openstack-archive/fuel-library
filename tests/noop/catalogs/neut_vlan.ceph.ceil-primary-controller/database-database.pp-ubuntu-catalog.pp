...
cs_resource { 'p_mysql':
  ...
  parameters      => {'socket' => '/var/run/mysqld/mysqld.sock', 'test_passwd' => '5eqwkxY3', 'test_user' => 'wsrep_sst'},
  primitive_class => 'ocf',
...
database_user { 'clustercheck@192.168.0.3':
  ...
  name          => 'clustercheck@192.168.0.3',
  password_hash => '*15F36E06A8026ECEF9ABA35D903A31A0FBF5D9ED',
  provider      => 'mysql',
...
exec { 'mysql_root_password':
  ...
  command => 'mysql -NBe "update mysql.user set password = password('5eqwkxY3') where user = 'root'"',
  creates => '/root/.my.cnf',
  path    => '/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin',
}
...
exec { 'set_mysql_rootpw':
  ...
  command   => 'mysqladmin -u root  password 5eqwkxY3',
  logoutput => 'true',
  path      => '/usr/local/sbin:/usr/bin:/usr/local/bin',
  tries     => '10',
  try_sleep => '3',
  unless    => 'mysqladmin -u root -p5eqwkxY3 status > /dev/null',
}

exec { 'wait-for-synced-state':
  before    => ['Exec[set_mysql_rootpw]', 'Exec[rm-init-file]'],
  command   => '/usr/bin/mysql -uwsrep_sst -p5eqwkxY3 -Nbe "show status like 'wsrep_local_state_comment'" | /bin/grep -q Synced && sleep 10',
  path      => '/usr/bin:/bin:/usr/sbin:/sbin',
  tries     => '60',
  try_sleep => '5',
}
...
exec { 'wait-initial-sync':
  ...
  command     => '/usr/bin/mysql -uwsrep_sst -p5eqwkxY3 -Nbe "show status like 'wsrep_local_state_comment'" | /bin/grep -q -e Synced -e Initialized && sleep 10',
  path        => '/usr/bin:/bin:/usr/sbin:/sbin',
  refreshonly => 'true',
  tries       => '60',
  try_sleep   => '5',
...
wsrep_sst_method=xtrabackup-v2
wsrep_sst_auth=wsrep_sst:5eqwkxY3
...
grant all on *.* to 'wsrep_sst'@'%' identified by '5eqwkxY3';
grant all on *.* to 'wsrep_sst'@'localhost' identified by '5eqwkxY3';
flush privileges;
