...
class { 'Openstack::Swift::Proxy':
  ...
  swift_proxy_local_ipaddr         => '192.168.0.3',
  swift_replication_local_ipaddr   => '192.168.1.3',
...
class { 'Openstack::Swift::Status':
  ...
  con_timeout => '5',
  endpoint    => 'http://192.168.0.3:8080',
...
class { 'Openstack::Swift::Storage_node':
  ...
  storage_weight              => '1',
  swift_hash_suffix           => 'swift_secret',
  swift_local_net_ip          => '192.168.1.3',
  swift_max_header_size       => '32768',
  swift_zone                  => '1',
  sync_rings                  => 'true',
...
class { 'Swift::Proxy':
  ...
  port                     => '8080',
  proxy_local_net_ip       => '192.168.0.3',
  workers                  => '4',
}
...
class { 'Swift::Storage::All':
  ...
  object_port          => '6000',
  outgoing_chmod       => '0644',
  storage_local_net_ip => '192.168.1.3',
}
...
exec { 'rsync /etc/swift/account.ring.gz':
  command => 'rsync -q -a rsync://192.168.1.2/swift_server/account.ring.gz /etc/swift/account.ring.gz',
  onlyif  => 'test `rsync --dry-run --itemize-changes -a rsync://192.168.1.2/swift_server/account.ring.gz /etc/swift/account.ring.gz | wc -l` -gt 0',
...
exec { 'rsync /etc/swift/backups/':
  command => 'rsync -q -a -r rsync://192.168.1.2/swift_server/backups/ /etc/swift/backups/',
  onlyif  => 'test `rsync --dry-run --itemize-changes -a -r rsync://192.168.1.2/swift_server/backups/ /etc/swift/backups/ | wc -l` -gt 0',
  path    => ['/bin', '/usr/bin', '/usr/local/bin'],
  timeout => '900',
  user    => 'root',
}
...
exec { 'rsync /etc/swift/container.ring.gz':
  command => 'rsync -q -a rsync://192.168.1.2/swift_server/container.ring.gz /etc/swift/container.ring.gz',
  onlyif  => 'test `rsync --dry-run --itemize-changes -a rsync://192.168.1.2/swift_server/container.ring.gz /etc/swift/container.ring.gz | wc -l` -gt 0',
  path    => ['/bin', '/usr/bin', '/usr/local/bin'],
  timeout => '900',
  user    => 'root',
}
...
exec { 'rsync /etc/swift/object.ring.gz':
  command => 'rsync -q -a rsync://192.168.1.2/swift_server/object.ring.gz /etc/swift/object.ring.gz',
  onlyif  => 'test `rsync --dry-run --itemize-changes -a rsync://192.168.1.2/swift_server/object.ring.gz /etc/swift/object.ring.gz | wc -l` -gt 0',
  path    => ['/bin', '/usr/bin', '/usr/local/bin'],
  timeout => '900',
  user    => 'root',
}
...
service swiftcheck
{
        ...
        bind            = 0.0.0.0
        server_args     = http://192.168.0.3:8080 192.168.0.2 5
        only_from       = 127.0.0.1 240.0.0.2 192.168.1.0/24 192.168.0.0/24
        per_source      = UNLIMITED
        cps             = 512 10
        flags           = IPv4
...
rsync::get { '/etc/swift/account.ring.gz':
  ...
  source   => 'rsync://192.168.1.2/swift_server/account.ring.gz',
  timeout  => '900',
}
...
rsync::get { '/etc/swift/backups/':
  ...
  path      => '/etc/swift/backups/',
  recursive => 'true',
  source    => 'rsync://192.168.1.2/swift_server/backups/',
  timeout   => '900',
}
...
rsync::get { '/etc/swift/container.ring.gz':
  ...
  source   => 'rsync://192.168.1.2/swift_server/container.ring.gz',
  timeout  => '900',
}
...
rsync::get { '/etc/swift/object.ring.gz':
  ...
  source   => 'rsync://192.168.1.2/swift_server/object.ring.gz',
  timeout  => '900',
}
