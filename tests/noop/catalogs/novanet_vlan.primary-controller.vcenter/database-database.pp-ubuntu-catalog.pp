...
class { 'Galera':
  ...
  gcomm_port           => '4567',
  name                 => 'Galera',
  node_address         => '172.16.1.5',
  node_addresses       => ['172.16.1.6', '172.16.1.3', '172.16.1.5'],
...
class { 'Mysql::Server':
  ...
  galera_node_address     => '172.16.1.5',
  galera_nodes            => ['172.16.1.6', '172.16.1.3', '172.16.1.5'],
...
class { 'Openstack::Galera::Status':
  address         => '0.0.0.0',
  backend_host    => '172.16.1.5',
  backend_port    => '3307',
  backend_timeout => '10',
  before          => 'Haproxy_backend_status[mysql]',
  mysql_module    => '0.9',
  name            => 'Openstack::Galera::Status',
  only_from       => '127.0.0.1 240.0.0.2 172.16.1.0/24',
  port            => '49000',
  status_allow    => '172.16.1.5',
  status_password => 'vugKPCKR',
...
class { 'Osnailyfacter::Mysql_user':
  access_networks => ['localhost', '127.0.0.1', '240.0.0.0/255.255.0.0', '172.16.1.0/255.255.255.0'],
...
cs_resource { 'p_mysql':
  ...
  parameters      => {'socket' => '/var/run/mysqld/mysqld.sock', 'test_passwd' => '4t67JmJk', 'test_user' => 'wsrep_sst'},
  primitive_class => 'ocf',
  primitive_type  => 'mysql-wss',
  provided_by     => 'fuel',
}

database_grant { 'clustercheck@172.16.1.5/*.*':
  name       => 'clustercheck@172.16.1.5/*.*',
  privileges => 'Status_priv',
}

database_user { 'clustercheck@172.16.1.5':
  ensure        => 'present',
  before        => 'Database_grant[clustercheck@172.16.1.5/*.*]',
  name          => 'clustercheck@172.16.1.5',
  password_hash => '*D45E7523A385CFE3C67927DC8CC7953E60EBE226',
  provider      => 'mysql',
...
exec { 'mysql_drop_test':
  before  => ['Osnailyfacter::Mysql_grant[localhost]', 'Osnailyfacter::Mysql_grant[127.0.0.1]', 'Osnailyfacter::Mysql_grant[240.0.0.0/255.255.0.0]', 'Osnailyfacter::Mysql_grant[172.16.1.0/255.255.255.0]'],
...
exec { 'mysql_root_172.16.1.0/255.255.255.0':
  command => 'mysql -NBe "grant all on *.* to 'root'@'172.16.1.0/255.255.255.0' with grant option"',
  creates => '/root/.my.cnf',
  path    => '/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin',
}
...
exec { 'mysql_root_password':
  ...
  command => 'mysql -NBe "update mysql.user set password = password('4t67JmJk') where user = 'root'"',
  creates => '/root/.my.cnf',
  path    => '/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin',
}
...
exec { 'set_mysql_rootpw':
  ...
  command   => 'mysqladmin -u root  password 4t67JmJk',
  logoutput => 'true',
  path      => '/usr/local/sbin:/usr/bin:/usr/local/bin',
  tries     => '10',
  try_sleep => '3',
  unless    => 'mysqladmin -u root -p4t67JmJk status > /dev/null',
}

exec { 'wait-for-synced-state':
  before    => ['Exec[set_mysql_rootpw]', 'Exec[rm-init-file]'],
  command   => '/usr/bin/mysql -uwsrep_sst -p4t67JmJk -Nbe "show status like 'wsrep_local_state_comment'" | /bin/grep -q Synced && sleep 10',
  path      => '/usr/bin:/bin:/usr/sbin:/sbin',
  tries     => '60',
  try_sleep => '5',
}
...
exec { 'wait-initial-sync':
  ...
  command     => '/usr/bin/mysql -uwsrep_sst -p4t67JmJk -Nbe "show status like 'wsrep_local_state_comment'" | /bin/grep -q -e Synced -e Initialized && sleep 10',
  path        => '/usr/bin:/bin:/usr/sbin:/sbin',
  refreshonly => 'true',
  tries       => '60',
  try_sleep   => '5',
...
wsrep_cluster_address="gcomm://172.16.1.6:4567,172.16.1.3:4567,172.16.1.5:4567?pc.wait_prim=no"
wsrep_provider=/usr/lib/galera/libgalera_smm.so
wsrep_cluster_name="openstack"

wsrep_slave_threads=8
wsrep_sst_method=xtrabackup-v2
wsrep_sst_auth=wsrep_sst:4t67JmJk
wsrep_node_address=172.16.1.5
wsrep_provider_options="gcache.size = 320M"
wsrep_provider_options="gmcast.listen_addr = tcp://172.16.1.5:4567"
...
MYSQL_PASSWORD="vugKPCKR"
MYSQL_HOST="172.16.1.5"
MYSQL_PORT="3307"
...
grant all on *.* to 'wsrep_sst'@'%' identified by '4t67JmJk';
grant all on *.* to 'wsrep_sst'@'localhost' identified by '4t67JmJk';
flush privileges;
...
file { 'mysql_password':
  before  => 'Database_user[clustercheck@172.16.1.5]',
  content => '[client]
user=root
host=localhost
password=4t67JmJk
...
osnailyfacter::mysql_grant { '172.16.1.0/255.255.255.0':
  before  => 'Exec[mysql_root_password]',
  name    => '172.16.1.0/255.255.255.0',
