...
class { 'Galera':
  ...
  gcomm_port           => '4567',
  name                 => 'Galera',
  node_address         => '192.168.0.3',
  node_addresses       => '192.168.0.3',
  ...
  wsrep_sst_password   => 'POvhJ6iQOcf9d4TCsul2ZRQI',
}
...
class { 'Mysql::Server':
  ...
  galera_gcache_factor    => '2',
  galera_node_address     => '192.168.0.3',
  galera_nodes            => '192.168.0.3',
...
class { 'Openstack::Galera::Status':
  ...
  port            => '49000',
  status_allow    => '192.168.0.3',
  status_password => 'XV0HBh9vJSqHOp8782RZRBgU',
...
cs_resource { 'p_mysql':
  ...
  parameters      => {'socket' => '/var/run/mysqld/mysqld.sock', 'test_passwd' => 'POvhJ6iQOcf9d4TCsul2ZRQI', 'test_user' => 'wsrep_sst'},
  primitive_class => 'ocf',
  primitive_type  => 'mysql-wss',
  provided_by     => 'fuel',
}

database_grant { 'clustercheck@192.168.0.3/*.*':
  name       => 'clustercheck@192.168.0.3/*.*',
  privileges => 'Status_priv',
}

database_user { 'clustercheck@192.168.0.3':
  ensure        => 'present',
  before        => 'Database_grant[clustercheck@192.168.0.3/*.*]',
  name          => 'clustercheck@192.168.0.3',
  password_hash => '*FDA88354E6B614C2D4255D59A8BDEA5040DBE49F',
  provider      => 'mysql',
...
exec { 'mysql_root_password':
  ...
  command => 'mysql -NBe "update mysql.user set password = password('POvhJ6iQOcf9d4TCsul2ZRQI') where user = 'root'"',
  creates => '/root/.my.cnf',
  path    => '/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin',
}
...
exec { 'set_mysql_rootpw':
  ...
  command   => 'mysqladmin -u root  password POvhJ6iQOcf9d4TCsul2ZRQI',
  logoutput => 'true',
  path      => '/usr/local/sbin:/usr/bin:/usr/local/bin',
  tries     => '10',
  try_sleep => '3',
  unless    => 'mysqladmin -u root -pPOvhJ6iQOcf9d4TCsul2ZRQI status > /dev/null',
}

exec { 'wait-for-synced-state':
  before    => ['Exec[set_mysql_rootpw]', 'Exec[rm-init-file]'],
  command   => '/usr/bin/mysql -uwsrep_sst -pPOvhJ6iQOcf9d4TCsul2ZRQI -Nbe "show status like 'wsrep_local_state_comment'" | /bin/grep -q Synced && sleep 10',
  path      => '/usr/bin:/bin:/usr/sbin:/sbin',
  tries     => '60',
  try_sleep => '5',
}
...
exec { 'wait-initial-sync':
  ...
  command     => '/usr/bin/mysql -uwsrep_sst -pPOvhJ6iQOcf9d4TCsul2ZRQI -Nbe "show status like 'wsrep_local_state_comment'" | /bin/grep -q -e Synced -e Initialized && sleep 10',
  path        => '/usr/bin:/bin:/usr/sbin:/sbin',
  refreshonly => 'true',
  tries       => '60',
  try_sleep   => '5',
...
wsrep_cluster_address="gcomm://192.168.0.3:4567?pc.wait_prim=no"
wsrep_provider=/usr/lib/galera/libgalera_smm.so
wsrep_cluster_name="openstack"

wsrep_slave_threads=8
wsrep_sst_method=xtrabackup-v2
wsrep_sst_auth=wsrep_sst:POvhJ6iQOcf9d4TCsul2ZRQI
wsrep_node_address=192.168.0.3
wsrep_provider_options="gcache.size = 256M"
wsrep_provider_options="gmcast.listen_addr = tcp://192.168.0.3:4567"
...
MYSQL_PASSWORD="XV0HBh9vJSqHOp8782RZRBgU"
...
grant all on *.* to 'wsrep_sst'@'%' identified by 'POvhJ6iQOcf9d4TCsul2ZRQI';
grant all on *.* to 'wsrep_sst'@'localhost' identified by 'POvhJ6iQOcf9d4TCsul2ZRQI';
flush privileges;
...
file { 'mysql_password':
  before  => 'Database_user[clustercheck@192.168.0.3]',
  content => '[client]
user=root
host=localhost
password=POvhJ6iQOcf9d4TCsul2ZRQI
