...
concat::fragment { 'swift_balancermember_swift':
  ...
  content => '  server node-1 192.168.0.3:8080  check port 49001 inter 15s fastinter 2s downinter 8s rise 3 fall 3
',
...
concat::fragment { 'swift_listen_block':
  content => '
listen swift
  bind 172.16.51.116:8080 ssl crt /var/lib/astute/haproxy/public_haproxy.pem
...
haproxy::listen { 'swift':
  bind             => {'172.16.51.116:8080' => ['ssl', 'crt', '/var/lib/astute/haproxy/public_haproxy.pem'], '192.168.0.2:8080' => ''},
  bind_options     => '',
