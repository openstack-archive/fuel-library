...
concat::fragment { 'horizon-ssl_balancermember_horizon-ssl':
  ...
  content => '  server node-128 192.168.0.2:80  weight 1 check
  server node-129 192.168.0.3:80  weight 1 check
  server node-131 192.168.0.4:80  weight 1 check
',
...
concat::fragment { 'horizon-ssl_listen_block':
  content => '
listen horizon-ssl
  bind 10.109.1.2:443 ssl crt /var/lib/astute/haproxy/public_haproxy.pem
  balance  source
  mode  http
  option  forwardfor
  option  httpchk
