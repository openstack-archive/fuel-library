...
class { 'Murano':
  ...
  rabbit_os_hosts      => ['192.168.0.2:5673', ' 192.168.0.4:5673', ' 192.168.0.3:5673'],
...
exec { 'murano_import_package_io.murano':
  command     => 'murano package-import '/var/cache/murano/meta/io.murano.zip' --is-public --exists-action u',
  environment => ['OS_TENANT_NAME=services', 'OS_USERNAME=murano', 'OS_PASSWORD=xP8WtHQw', 'OS_AUTH_URL=https://public.fuel.local:5000/v2.0/', 'OS_ENDPOINT_TYPE=internalURL', 'OS_REGION_NAME=RegionOne'],
  group       => 'root',
  path        => '/sbin:/usr/sbin:/bin:/usr/bin',
  tries       => '3',
  try_sleep   => '10',
  unless      => 'murano package-list 2>&1 | grep -q ' io.murano '',
  user        => 'root',
}
...
file_line { 'murano_url':
  ...
  line   => 'MURANO_API_URL = 'http://192.168.0.2:8082'',
  name   => 'murano_url',
  path   => '/etc/openstack-dashboard/local_settings.py',
  tag    => 'patch-horizon-config',
}
...
haproxy_backend_status { 'murano-api':
  before => 'Murano::Application[io.murano]',
  name   => 'murano-api',
  url    => 'http://192.168.0.2:10000/;csv',
}
...
service { 'murano-api':
  ...
  before  => ['Murano::Application[io.murano]', 'Haproxy_backend_status[murano-api]'],
  enable  => 'true',
  name    => 'murano-api',
