...
',
  ...
  scriptaliases               => [],
  serveraliases               => 'node-137.test.domain.local',
...
', 'default_vhost' => 'true', 'headers' => ['set X-XSS-Protection "1; mode=block"', 'set X-Content-Type-Options nosniff', 'always append X-Frame-Options SAMEORIGIN'], 'options' => '-Indexes', 'setenvif' => 'X-Forwarded-Proto https HTTPS=1'},
  ...
  server_aliases      => 'node-137.test.domain.local',
  servername          => 'node-137.test.domain.local',
  ssl_redirect        => 'true',
...
class { 'Horizon':
  ...
  keystone_url                   => 'http://192.168.0.5:5000/v2.0',
  listen_ssl                     => 'false',
...
concat::fragment { 'horizon_ssl_vhost-serveralias':
  content => '
  ## Server aliases
  ServerAlias node-137.test.domain.local
',
...
OPENSTACK_KEYSTONE_URL = "http://192.168.0.5:5000/v2.0"
