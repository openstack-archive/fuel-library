...
concat::fragment { 'keystone-1_balancermember_keystone-1':
  ...
  content => '  server node-137 192.168.0.4:5000  check inter 10s fastinter 2s downinter 2s rise 30 fall 3
',
...
concat::fragment { 'keystone-1_listen_block':
  content => '
listen keystone-1
  bind 172.16.0.6:5000 ssl crt /var/lib/astute/haproxy/public_haproxy.pem
  bind 192.168.0.5:5000 
  option  httpchk
  option  httplog
  option  httpclose
',
...
concat::fragment { 'keystone-2_balancermember_keystone-2':
  ...
  content => '  server node-137 192.168.0.4:35357  check inter 10s fastinter 2s downinter 2s rise 30 fall 3
',
...
haproxy::listen { 'keystone-1':
  bind             => {'172.16.0.6:5000' => ['ssl', 'crt', '/var/lib/astute/haproxy/public_haproxy.pem'], '192.168.0.5:5000' => ''},
  bind_options     => '',
