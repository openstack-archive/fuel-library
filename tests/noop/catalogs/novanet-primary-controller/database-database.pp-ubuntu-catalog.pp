...
class { 'Galera':
  ...
  gcomm_port           => '4567',
  name                 => 'Galera',
  node_address         => '192.168.0.4',
  node_addresses       => '192.168.0.4',
...
class { 'Mysql::Server':
  ...
  galera_gcache_factor    => '3',
  galera_node_address     => '192.168.0.4',
  galera_nodes            => '192.168.0.4',
...
class { 'Openstack::Galera::Status':
  ...
  port            => '49000',
  status_allow    => '192.168.0.4',
  status_password => 'jMCDynJR',
...
cs_resource { 'p_mysql':
  ...
  parameters      => {'socket' => '/var/run/mysqld/mysqld.sock', 'test_passwd' => 'M3VTf8U9', 'test_user' => 'wsrep_sst'},
  primitive_class => 'ocf',
  primitive_type  => 'mysql-wss',
  provided_by     => 'fuel',
}

database_grant { 'clustercheck@192.168.0.4/*.*':
  name       => 'clustercheck@192.168.0.4/*.*',
  privileges => 'Status_priv',
}

database_user { 'clustercheck@192.168.0.4':
  ensure        => 'present',
  before        => 'Database_grant[clustercheck@192.168.0.4/*.*]',
  name          => 'clustercheck@192.168.0.4',
  password_hash => '*271C6A39FF32FE223F159D136A64E10936F363C4',
  provider      => 'mysql',
...
exec { 'mysql_root_password':
  ...
  command => 'mysql -NBe "update mysql.user set password = password('M3VTf8U9') where user = 'root'"',
  creates => '/root/.my.cnf',
  path    => '/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin',
}
...
exec { 'set_mysql_rootpw':
  ...
  command   => 'mysqladmin -u root  password M3VTf8U9',
  logoutput => 'true',
  path      => '/usr/local/sbin:/usr/bin:/usr/local/bin',
  tries     => '10',
  try_sleep => '3',
  unless    => 'mysqladmin -u root -pM3VTf8U9 status > /dev/null',
}

exec { 'wait-for-synced-state':
  before    => ['Exec[set_mysql_rootpw]', 'Exec[rm-init-file]'],
  command   => '/usr/bin/mysql -uwsrep_sst -pM3VTf8U9 -Nbe "show status like 'wsrep_local_state_comment'" | /bin/grep -q Synced && sleep 10',
  path      => '/usr/bin:/bin:/usr/sbin:/sbin',
  tries     => '60',
  try_sleep => '5',
}
...
exec { 'wait-initial-sync':
  ...
  command     => '/usr/bin/mysql -uwsrep_sst -pM3VTf8U9 -Nbe "show status like 'wsrep_local_state_comment'" | /bin/grep -q -e Synced -e Initialized && sleep 10',
  path        => '/usr/bin:/bin:/usr/sbin:/sbin',
  refreshonly => 'true',
  tries       => '60',
  try_sleep   => '5',
...
wsrep_cluster_address="gcomm://192.168.0.4:4567?pc.wait_prim=no"
wsrep_provider=/usr/lib/galera/libgalera_smm.so
wsrep_cluster_name="openstack"

wsrep_slave_threads=8
wsrep_sst_method=xtrabackup-v2
wsrep_sst_auth=wsrep_sst:M3VTf8U9
wsrep_node_address=192.168.0.4
wsrep_provider_options="gcache.size = 256M"
wsrep_provider_options="gmcast.listen_addr = tcp://192.168.0.4:4567"
...
grant all on *.* to 'wsrep_sst'@'%' identified by 'M3VTf8U9';
grant all on *.* to 'wsrep_sst'@'localhost' identified by 'M3VTf8U9';
flush privileges;
...
file { 'mysql_password':
  before  => 'Database_user[clustercheck@192.168.0.4]',
  content => '[client]
user=root
host=localhost
password=M3VTf8U9
