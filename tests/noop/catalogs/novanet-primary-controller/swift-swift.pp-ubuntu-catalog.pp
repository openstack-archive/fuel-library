...
class { 'Openstack::Swift::Proxy':
  ...
  master_swift_proxy_ip            => '192.168.0.4',
  master_swift_replication_ip      => '192.168.1.4',
  ...
  swift_proxies_cache              => '192.168.0.4',
  swift_proxy_local_ipaddr         => '192.168.0.4',
  swift_replication_local_ipaddr   => '192.168.1.4',
  swift_user_password              => 'UcPlc9Wp',
  verbose                          => 'true',
}
...
class { 'Openstack::Swift::Storage_node':
  ...
  storage_weight              => '1',
  swift_hash_suffix           => 'swift_secret',
  swift_local_net_ip          => '192.168.1.4',
  swift_max_header_size       => '32768',
  swift_zone                  => '1',
  sync_rings                  => 'false',
...
class { 'Swift::Proxy':
  ...
  port                     => '8080',
  proxy_local_net_ip       => '192.168.0.4',
  workers                  => '4',
}
...
class { 'Swift::Storage::All':
  ...
  object_port          => '6000',
  outgoing_chmod       => '0644',
  storage_local_net_ip => '192.168.1.4',
}
...
exec { 'swift-dispersion-populate':
  ...
  onlyif    => 'swift -A http://192.168.0.5:5000/v2.0/ -U services:swift -K UcPlc9Wp -V 2.0 stat | grep 'Account: '',
  path      => ['/bin', '/usr/bin'],
  require   => 'Package[swiftclient]',
  subscribe => 'File[/etc/swift/dispersion.conf]',
  timeout   => '0',
  unless    => 'swift -A http://192.168.0.5:5000/v2.0/ -U services:swift -K UcPlc9Wp -V 2.0 list | grep dispersion_',
}
...
service swiftcheck
{
        ...
        bind            = 0.0.0.0
        server_args     = http://192.168.0.4:8080 192.168.0.5 5
        only_from       = 127.0.0.1 240.0.0.2 192.168.1.0/24 192.168.0.0/24
        per_source      = UNLIMITED
        cps             = 512 10
        flags           = IPv4
...
ring_devices { 'all':
  ...
  require  => 'Class[Swift]',
  storages => {'node-137' => {'fqdn' => 'node-137.test.domain.local', 'name' => 'node-137', 'node_roles' => ['primary-controller'], 'storage_address' => '192.168.1.4', 'swift_zone' => '1', 'uid' => '137', 'user_node_name' => 'Untitled (6a:e7)'}},
}
