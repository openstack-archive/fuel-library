...
class { 'Galera':
  ...
  gcomm_port           => '4567',
  name                 => 'Galera',
  node_address         => '10.122.12.3',
  node_addresses       => '10.122.12.3',
...
class { 'Mysql::Server':
  ...
  galera_gcache_factor    => '4',
  galera_node_address     => '10.122.12.3',
  galera_nodes            => '10.122.12.3',
...
class { 'Openstack::Galera::Status':
  address         => '0.0.0.0',
  backend_host    => '10.122.12.3',
  backend_port    => '3307',
  backend_timeout => '10',
  before          => 'Haproxy_backend_status[mysql]',
  mysql_module    => '0.9',
  name            => 'Openstack::Galera::Status',
  only_from       => '127.0.0.1 240.0.0.2 10.122.12.0/24',
  port            => '49000',
  status_allow    => '10.122.12.3',
  status_password => 'qEYkmkV7',
...
class { 'Osnailyfacter::Mysql_user':
  access_networks => ['localhost', '127.0.0.1', '240.0.0.0/255.255.0.0', '10.122.12.0/255.255.255.0'],
...
cs_resource { 'p_mysql':
  ...
  parameters      => {'socket' => '/var/run/mysqld/mysqld.sock', 'test_passwd' => 'sx2tGnw7', 'test_user' => 'wsrep_sst'},
  primitive_class => 'ocf',
  primitive_type  => 'mysql-wss',
  provided_by     => 'fuel',
}

database_grant { 'clustercheck@10.122.12.3/*.*':
  name       => 'clustercheck@10.122.12.3/*.*',
  privileges => 'Status_priv',
}

database_user { 'clustercheck@10.122.12.3':
  ensure        => 'present',
  before        => 'Database_grant[clustercheck@10.122.12.3/*.*]',
  name          => 'clustercheck@10.122.12.3',
  password_hash => '*61D9E61F79AD82CF9BF74913DD5BF92406030F96',
  provider      => 'mysql',
...
exec { 'mysql_drop_test':
  before  => ['Osnailyfacter::Mysql_grant[localhost]', 'Osnailyfacter::Mysql_grant[127.0.0.1]', 'Osnailyfacter::Mysql_grant[240.0.0.0/255.255.0.0]', 'Osnailyfacter::Mysql_grant[10.122.12.0/255.255.255.0]'],
...
exec { 'mysql_root_10.122.12.0/255.255.255.0':
  command => 'mysql -NBe "grant all on *.* to 'root'@'10.122.12.0/255.255.255.0' with grant option"',
  creates => '/root/.my.cnf',
  path    => '/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin',
}
...
exec { 'mysql_root_password':
  ...
  command => 'mysql -NBe "update mysql.user set password = password('sx2tGnw7') where user = 'root'"',
  creates => '/root/.my.cnf',
  path    => '/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin',
}
...
exec { 'set_mysql_rootpw':
  ...
  command   => 'mysqladmin -u root  password sx2tGnw7',
  logoutput => 'true',
  path      => '/usr/local/sbin:/usr/bin:/usr/local/bin',
  tries     => '10',
  try_sleep => '3',
  unless    => 'mysqladmin -u root -psx2tGnw7 status > /dev/null',
}

exec { 'wait-for-synced-state':
  before    => ['Exec[set_mysql_rootpw]', 'Exec[rm-init-file]'],
  command   => '/usr/bin/mysql -uwsrep_sst -psx2tGnw7 -Nbe "show status like 'wsrep_local_state_comment'" | /bin/grep -q Synced && sleep 10',
  path      => '/usr/bin:/bin:/usr/sbin:/sbin',
  tries     => '60',
  try_sleep => '5',
}
...
exec { 'wait-initial-sync':
  ...
  command     => '/usr/bin/mysql -uwsrep_sst -psx2tGnw7 -Nbe "show status like 'wsrep_local_state_comment'" | /bin/grep -q -e Synced -e Initialized && sleep 10',
  path        => '/usr/bin:/bin:/usr/sbin:/sbin',
  refreshonly => 'true',
  tries       => '60',
  try_sleep   => '5',
...
wsrep_cluster_address="gcomm://10.122.12.3:4567?pc.wait_prim=no"
wsrep_provider=/usr/lib/galera/libgalera_smm.so
wsrep_cluster_name="openstack"

wsrep_slave_threads=8
wsrep_sst_method=xtrabackup-v2
wsrep_sst_auth=wsrep_sst:sx2tGnw7
wsrep_node_address=10.122.12.3
wsrep_provider_options="gcache.size = 256M"
wsrep_provider_options="gmcast.listen_addr = tcp://10.122.12.3:4567"
...
MYSQL_PASSWORD="qEYkmkV7"
MYSQL_HOST="10.122.12.3"
MYSQL_PORT="3307"
...
grant all on *.* to 'wsrep_sst'@'%' identified by 'sx2tGnw7';
grant all on *.* to 'wsrep_sst'@'localhost' identified by 'sx2tGnw7';
flush privileges;
...
file { 'mysql_password':
  before  => 'Database_user[clustercheck@10.122.12.3]',
  content => '[client]
user=root
host=localhost
password=sx2tGnw7
...
osnailyfacter::mysql_grant { '10.122.12.0/255.255.255.0':
  before  => 'Exec[mysql_root_password]',
  name    => '10.122.12.0/255.255.255.0',
  network => '10.122.12.0/255.255.255.0',
  user    => 'root',
}
