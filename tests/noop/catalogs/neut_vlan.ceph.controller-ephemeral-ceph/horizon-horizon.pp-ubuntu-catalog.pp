...
',
  ...
  scriptaliases               => [],
  serveraliases               => 'node-1.domain.local',
...
', 'default_vhost' => 'true', 'headers' => ['set X-XSS-Protection "1; mode=block"', 'set X-Content-Type-Options nosniff', 'always append X-Frame-Options SAMEORIGIN'], 'options' => '-Indexes', 'setenvif' => 'X-Forwarded-Proto https HTTPS=1'},
  ...
  server_aliases      => 'node-1.domain.local',
  servername          => 'node-1.domain.local',
  ssl_redirect        => 'true',
...
class { 'Horizon':
  ...
  keystone_url                   => 'http://10.122.12.2:5000/v2.0',
  listen_ssl                     => 'false',
...
concat::fragment { 'horizon_ssl_vhost-serveralias':
  content => '
  ## Server aliases
  ServerAlias node-1.domain.local
',
...
OPENSTACK_KEYSTONE_URL = "http://10.122.12.2:5000/v2.0"
