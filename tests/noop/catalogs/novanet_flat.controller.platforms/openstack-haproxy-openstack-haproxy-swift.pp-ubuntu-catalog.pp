...
concat::fragment { 'swift_balancermember_swift':
  ...
  content => '  server node-1 10.108.2.4:8080  check port 49001 inter 15s fastinter 2s downinter 8s rise 3 fall 3
  server node-2 10.108.2.5:8080  check port 49001 inter 15s fastinter 2s downinter 8s rise 3 fall 3
  server node-3 10.108.2.6:8080  check port 49001 inter 15s fastinter 2s downinter 8s rise 3 fall 3
',
...
concat::fragment { 'swift_listen_block':
  content => '
listen swift
  bind 10.108.1.2:8080 ssl crt /var/lib/astute/haproxy/public_haproxy.pem
  bind 10.108.2.2:8080 
  option  httpchk
  option  httplog
  option  httpclose
',
...
haproxy::listen { 'swift':
  bind             => {'10.108.1.2:8080' => ['ssl', 'crt', '/var/lib/astute/haproxy/public_haproxy.pem'], '10.108.2.2:8080' => ''},
  bind_options     => '',
