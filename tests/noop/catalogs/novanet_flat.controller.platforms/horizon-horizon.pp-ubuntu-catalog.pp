...
',
  ...
  scriptaliases               => [],
  serveraliases               => 'node-1.test.domain.local',
...
', 'default_vhost' => 'true', 'headers' => ['set X-XSS-Protection "1; mode=block"', 'set X-Content-Type-Options nosniff', 'always append X-Frame-Options SAMEORIGIN'], 'options' => '-Indexes', 'setenvif' => 'X-Forwarded-Proto https HTTPS=1'},
  ...
  server_aliases      => 'node-1.test.domain.local',
  servername          => 'node-1.test.domain.local',
  ssl_redirect        => 'true',
...
class { 'Horizon':
  ...
  keystone_url                   => 'http://10.108.2.2:5000/v2.0',
  listen_ssl                     => 'false',
...
concat::fragment { 'horizon_ssl_vhost-serveralias':
  content => '
  ## Server aliases
  ServerAlias node-1.test.domain.local
',
...
CACHES = {
    'default': {
        ...
        'LOCATION': [ '10.108.2.4:11211','10.108.2.5:11211','10.108.2.6:11211', ],
...
OPENSTACK_KEYSTONE_URL = "http://10.108.2.2:5000/v2.0"
