...
anchor { 'database-cluster':
  before => ['Class[Mysql::Password]', 'Service[mysql]'],
...
class { 'Galera':
  ...
  gcomm_port           => '4567',
  name                 => 'Galera',
  node_address         => '10.108.2.4',
  node_addresses       => ['10.108.2.4', '10.108.2.5', '10.108.2.6'],
...
class { 'Mysql::Server':
  ...
  galera_gcache_factor    => '6',
  galera_node_address     => '10.108.2.4',
  galera_nodes            => ['10.108.2.4', '10.108.2.5', '10.108.2.6'],
...
class { 'Openstack::Galera::Status':
  address         => '0.0.0.0',
  backend_host    => '10.108.2.4',
  backend_port    => '3307',
  backend_timeout => '10',
  before          => 'Haproxy_backend_status[mysql]',
  mysql_module    => '0.9',
  name            => 'Openstack::Galera::Status',
  only_from       => '127.0.0.1 240.0.0.2 10.108.2.0/24',
  port            => '49000',
  status_allow    => '10.108.2.4',
  status_password => '4JFDJzqR',
...
class { 'Osnailyfacter::Mysql_user':
  access_networks => ['localhost', '127.0.0.1', '240.0.0.0/255.255.0.0', '10.108.2.0/255.255.255.0'],
...
database_grant { 'clustercheck@10.108.2.4/*.*':
  name       => 'clustercheck@10.108.2.4/*.*',
  privileges => 'Status_priv',
}

database_user { 'clustercheck@10.108.2.4':
  ensure        => 'present',
  before        => 'Database_grant[clustercheck@10.108.2.4/*.*]',
  name          => 'clustercheck@10.108.2.4',
  password_hash => '*8112224F74862C0DC2B4E0640CD9D55A882F55D6',
  provider      => 'mysql',
...
exec { 'mysql_drop_test':
  before  => ['Osnailyfacter::Mysql_grant[localhost]', 'Osnailyfacter::Mysql_grant[127.0.0.1]', 'Osnailyfacter::Mysql_grant[240.0.0.0/255.255.0.0]', 'Osnailyfacter::Mysql_grant[10.108.2.0/255.255.255.0]'],
...
exec { 'mysql_root_10.108.2.0/255.255.255.0':
  command => 'mysql -NBe "grant all on *.* to 'root'@'10.108.2.0/255.255.255.0' with grant option"',
  creates => '/root/.my.cnf',
  path    => '/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin',
}
...
exec { 'mysql_root_password':
  ...
  command => 'mysql -NBe "update mysql.user set password = password('NDG84Pcc') where user = 'root'"',
  creates => '/root/.my.cnf',
  path    => '/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin',
}
...
exec { 'set_mysql_rootpw':
  ...
  command   => 'mysqladmin -u root  password NDG84Pcc',
  logoutput => 'true',
  path      => '/usr/local/sbin:/usr/bin:/usr/local/bin',
  tries     => '10',
  try_sleep => '3',
  unless    => 'mysqladmin -u root -pNDG84Pcc status > /dev/null',
}

exec { 'wait-for-synced-state':
  before    => ['Exec[set_mysql_rootpw]', 'Exec[rm-init-file]'],
  command   => '/usr/bin/mysql -uwsrep_sst -pNDG84Pcc -Nbe "show status like 'wsrep_local_state_comment'" | /bin/grep -q Synced && sleep 10',
  path      => '/usr/bin:/bin:/usr/sbin:/sbin',
  tries     => '60',
  try_sleep => '5',
}
...
exec { 'wait-initial-sync':
  ...
  command     => '/usr/bin/mysql -uwsrep_sst -pNDG84Pcc -Nbe "show status like 'wsrep_local_state_comment'" | /bin/grep -q -e Synced -e Initialized && sleep 10',
  path        => '/usr/bin:/bin:/usr/sbin:/sbin',
  refreshonly => 'true',
  tries       => '60',
  try_sleep   => '5',
...
wsrep_cluster_address="gcomm://10.108.2.4:4567,10.108.2.5:4567,10.108.2.6:4567?pc.wait_prim=no"
wsrep_provider=/usr/lib/galera/libgalera_smm.so
wsrep_cluster_name="openstack"

wsrep_slave_threads=8
wsrep_sst_method=xtrabackup-v2
wsrep_sst_auth=wsrep_sst:NDG84Pcc
wsrep_node_address=10.108.2.4
wsrep_provider_options="gcache.size = 384M"
wsrep_provider_options="gmcast.listen_addr = tcp://10.108.2.4:4567"
...
MYSQL_PASSWORD="4JFDJzqR"
MYSQL_HOST="10.108.2.4"
MYSQL_PORT="3307"
...
grant all on *.* to 'wsrep_sst'@'%' identified by 'NDG84Pcc';
grant all on *.* to 'wsrep_sst'@'localhost' identified by 'NDG84Pcc';
flush privileges;
...
file { 'mysql_password':
  before  => 'Database_user[clustercheck@10.108.2.4]',
  content => '[client]
user=root
host=localhost
password=NDG84Pcc
...
osnailyfacter::mysql_grant { '10.108.2.0/255.255.255.0':
  before  => 'Exec[mysql_root_password]',
  name    => '10.108.2.0/255.255.255.0',
...
service { 'mysql':
  ...
  before   => ['Exec[set_mysql_rootpw]', 'Anchor[database-cluster-done]', 'Exec[wait-initial-sync]'],
  enable   => 'true',
  name     => 'p_mysql',
  provider => 'pacemaker',
}
