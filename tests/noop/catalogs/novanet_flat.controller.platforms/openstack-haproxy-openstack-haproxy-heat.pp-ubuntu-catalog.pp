...
concat::fragment { 'heat-api-cfn_balancermember_heat-api-cfn':
  ...
  content => '  server node-1 10.108.2.4:8000  check inter 10s fastinter 2s downinter 3s rise 3 fall 3
  server node-2 10.108.2.5:8000  check inter 10s fastinter 2s downinter 3s rise 3 fall 3
  server node-3 10.108.2.6:8000  check inter 10s fastinter 2s downinter 3s rise 3 fall 3
',
...
concat::fragment { 'heat-api-cfn_listen_block':
  content => '
listen heat-api-cfn
  bind 10.108.1.2:8000 ssl crt /var/lib/astute/haproxy/public_haproxy.pem
  bind 10.108.2.2:8000 
  option  httpchk
  option  httplog
  option  httpclose
  timeout server  660s
...
concat::fragment { 'heat-api-cloudwatch_balancermember_heat-api-cloudwatch':
  ...
  content => '  server node-1 10.108.2.4:8003  check inter 10s fastinter 2s downinter 3s rise 3 fall 3
  server node-2 10.108.2.5:8003  check inter 10s fastinter 2s downinter 3s rise 3 fall 3
  server node-3 10.108.2.6:8003  check inter 10s fastinter 2s downinter 3s rise 3 fall 3
',
...
listen heat-api-cloudwatch
  bind 10.108.1.2:8003 ssl crt /var/lib/astute/haproxy/public_haproxy.pem
  bind 10.108.2.2:8003 
  option  httpchk
  option  httplog
  option  httpclose
  timeout server  660s
...
concat::fragment { 'heat-api_balancermember_heat-api':
  ...
  content => '  server node-1 10.108.2.4:8004  check inter 10s fastinter 2s downinter 3s rise 3 fall 3
  server node-2 10.108.2.5:8004  check inter 10s fastinter 2s downinter 3s rise 3 fall 3
  server node-3 10.108.2.6:8004  check inter 10s fastinter 2s downinter 3s rise 3 fall 3
',
...
concat::fragment { 'heat-api_listen_block':
  content => '
listen heat-api
  bind 10.108.1.2:8004 ssl crt /var/lib/astute/haproxy/public_haproxy.pem
  bind 10.108.2.2:8004 
...
haproxy::listen { 'heat-api-cfn':
  bind             => {'10.108.1.2:8000' => ['ssl', 'crt', '/var/lib/astute/haproxy/public_haproxy.pem'], '10.108.2.2:8000' => ''},
  bind_options     => '',
...
haproxy::listen { 'heat-api-cloudwatch':
  bind             => {'10.108.1.2:8003' => ['ssl', 'crt', '/var/lib/astute/haproxy/public_haproxy.pem'], '10.108.2.2:8003' => ''},
  bind_options     => '',
  collect_exported => 'true',
  name             => 'heat-api-cloudwatch',
  notify           => 'Exec[haproxy-restart]',
...
haproxy::listen { 'heat-api':
  bind             => {'10.108.1.2:8004' => ['ssl', 'crt', '/var/lib/astute/haproxy/public_haproxy.pem'], '10.108.2.2:8004' => ''},
  bind_options     => '',
  collect_exported => 'true',
  name             => 'heat-api',
  notify           => 'Exec[haproxy-restart]',
