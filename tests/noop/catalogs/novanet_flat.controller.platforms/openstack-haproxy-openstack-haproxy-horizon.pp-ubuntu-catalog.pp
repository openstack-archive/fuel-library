...
concat::fragment { 'horizon-ssl_balancermember_horizon-ssl':
  ...
  content => '  server node-1 10.108.2.4:80  weight 1 check
  server node-2 10.108.2.5:80  weight 1 check
  server node-3 10.108.2.6:80  weight 1 check
',
...
concat::fragment { 'horizon-ssl_listen_block':
  content => '
listen horizon-ssl
  bind 10.108.1.2:443 ssl crt /var/lib/astute/haproxy/public_haproxy.pem
  balance  source
  mode  http
  option  forwardfor
  option  httpchk
