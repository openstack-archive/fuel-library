...
cs_colocation { 'conntrackd-with-pub-vip':
  name       => 'conntrackd-with-pub-vip',
  primitives => ['master_p_conntrackd:Master', 'vip__vrouter_pub'],
}

cs_resource { 'p_conntrackd':
  ensure          => 'present',
  before          => 'Service[p_conntrackd]',
  complex_type    => 'master',
  metadata        => {'failure-timeout' => '180s', 'migration-threshold' => 'INFINITY'},
  ms_metadata     => {'clone-node-max' => '1', 'interleave' => 'true', 'master-max' => '1', 'master-node-max' => '1', 'notify' => 'true', 'ordered' => 'false', 'target-role' => 'Master'},
  name            => 'p_conntrackd',
  operations      => {'monitor' => {'interval' => '30', 'timeout' => '60'}, 'monitor:Master' => {'interval' => '27', 'role' => 'Master', 'timeout' => '60'}},
  parameters      => {'bridge' => 'br-mgmt'},
  primitive_class => 'ocf',
...
file { '/etc/conntrackd/conntrackd.conf':
  before  => ['Cs_resource[p_conntrackd]', 'Cs_resource[p_conntrackd]'],
  content => '#
...
file { '/etc/logrotate.d/conntrackd':
  content => '/var/log/conntrackd-stats.log {
    daily
    rotate 1
    missingok
    copytruncate
...
package { 'conntrackd':
  ...
  before => ['File[/etc/conntrackd/conntrackd.conf]', 'File[/etc/logrotate.d/conntrackd]'],
  name   => 'conntrackd',
}

service { 'p_conntrackd':
  ensure   => 'running',
  before   => 'Cs_colocation[conntrackd-with-pub-vip]',
  enable   => 'true',
  name     => 'p_conntrackd',
  provider => 'pacemaker',
}
