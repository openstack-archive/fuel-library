...
class { 'Openstack::Swift::Proxy':
  ...
  master_swift_proxy_ip            => '10.108.2.4',
  master_swift_replication_ip      => '10.108.4.2',
  name                             => 'Openstack::Swift::Proxy',
  package_ensure                   => 'present',
  primary_proxy                    => 'false',
  proxy_account_autocreate         => 'true',
  proxy_allow_account_management   => 'true',
  proxy_pipeline                   => ['catch_errors', 'crossdomain', 'healthcheck', 'cache', 'bulk', 'tempurl', 'ratelimit', 'formpost', 'swift3', 's3token', 'authtoken', 'keystone', 'staticweb', 'container_quotas', 'account_quotas', 'slo', 'ceilometer', 'proxy-server'],
  proxy_port                       => '8080',
  proxy_workers                    => '4',
  ...
  swift_proxies_cache              => ['10.108.2.4', '10.108.2.5', '10.108.2.6'],
  swift_proxy_local_ipaddr         => '10.108.2.4',
  swift_replication_local_ipaddr   => '10.108.4.2',
  swift_user_password              => 'MdNIkfj3',
  verbose                          => 'true',
}
...
class { 'Openstack::Swift::Status':
  ...
  con_timeout => '5',
  endpoint    => 'http://10.108.2.4:8080',
  name        => 'Openstack::Swift::Status',
  only_from   => '127.0.0.1 240.0.0.2 10.108.4.0/24 10.108.2.0/24',
  port        => '49001',
  vip         => '10.108.2.2',
}
...
class { 'Swift::Proxy::Cache':
  memcache_servers => ['10.108.2.4:11211', '10.108.2.5:11211', '10.108.2.6:11211'],
...
class { 'Swift::Storage::All':
  ...
  object_port          => '6000',
  outgoing_chmod       => '0644',
  storage_local_net_ip => '10.108.4.2',
}
...
concat::fragment { 'swift_proxy':
  before  => ['Class[Swift::Proxy::Catch_errors]', 'Class[Swift::Proxy::Crossdomain]', 'Class[Swift::Proxy::Healthcheck]', 'Class[Swift::Proxy::Cache]', 'Class[Swift::Proxy::Bulk]', 'Class[Swift::Proxy::Tempurl]', 'Class[Swift::Proxy::Ratelimit]', 'Class[Swift::Proxy::Formpost]', 'Class[Swift::Proxy::Swift3]', 'Class[Swift::Proxy::S3token]', 'Class[Swift::Proxy::Authtoken]', 'Class[Swift::Proxy::Keystone]', 'Class[Swift::Proxy::Staticweb]', 'Class[Swift::Proxy::Container_quotas]', 'Class[Swift::Proxy::Account_quotas]', 'Class[Swift::Proxy::Slo]', 'Class[Swift::Proxy::Ceilometer]'],
...
exec { 'rsync /etc/swift/account.ring.gz':
  command => 'rsync -q -a rsync://10.108.4.2/swift_server/account.ring.gz /etc/swift/account.ring.gz',
  onlyif  => 'test `rsync --dry-run --itemize-changes -a rsync://10.108.4.2/swift_server/account.ring.gz /etc/swift/account.ring.gz | wc -l` -gt 0',
  path    => ['/bin', '/usr/bin', '/usr/local/bin'],
  timeout => '900',
  user    => 'root',
}

exec { 'rsync /etc/swift/backups/':
  command => 'rsync -q -a -r rsync://10.108.4.2/swift_server/backups/ /etc/swift/backups/',
  onlyif  => 'test `rsync --dry-run --itemize-changes -a -r rsync://10.108.4.2/swift_server/backups/ /etc/swift/backups/ | wc -l` -gt 0',
  path    => ['/bin', '/usr/bin', '/usr/local/bin'],
  timeout => '900',
  user    => 'root',
}

exec { 'rsync /etc/swift/container.ring.gz':
  command => 'rsync -q -a rsync://10.108.4.2/swift_server/container.ring.gz /etc/swift/container.ring.gz',
  onlyif  => 'test `rsync --dry-run --itemize-changes -a rsync://10.108.4.2/swift_server/container.ring.gz /etc/swift/container.ring.gz | wc -l` -gt 0',
  path    => ['/bin', '/usr/bin', '/usr/local/bin'],
  timeout => '900',
  user    => 'root',
}

exec { 'rsync /etc/swift/object.ring.gz':
  command => 'rsync -q -a rsync://10.108.4.2/swift_server/object.ring.gz /etc/swift/object.ring.gz',
  onlyif  => 'test `rsync --dry-run --itemize-changes -a rsync://10.108.4.2/swift_server/object.ring.gz /etc/swift/object.ring.gz | wc -l` -gt 0',
  path    => ['/bin', '/usr/bin', '/usr/local/bin'],
  timeout => '900',
  user    => 'root',
}
...
exec { 'swift-dispersion-populate':
  ...
  onlyif    => 'swift -A http://10.108.2.2:5000/v2.0/ -U services:swift -K MdNIkfj3 -V 2.0 stat | grep 'Account: '',
  path      => ['/bin', '/usr/bin'],
  require   => 'Package[swiftclient]',
  subscribe => 'File[/etc/swift/dispersion.conf]',
  timeout   => '0',
  unless    => 'swift -A http://10.108.2.2:5000/v2.0/ -U services:swift -K MdNIkfj3 -V 2.0 list | grep dispersion_',
}
...
service swiftcheck
{
        ...
        bind            = 0.0.0.0
        server_args     = http://10.108.2.4:8080 10.108.2.2 5
        only_from       = 127.0.0.1 240.0.0.2 10.108.4.0/24 10.108.2.0/24
        per_source      = UNLIMITED
        cps             = 512 10
        flags           = IPv4
...
file { '/tmp//_etc_swift_proxy-server.conf/fragments/33_swift_ceilometer':
  ensure  => 'file',
  alias   => 'concat_fragment_swift_ceilometer',
  content => '
[filter:ceilometer]
use = egg:ceilometer#swift
',
  mode    => '0640',
  notify  => 'Exec[concat_/etc/swift/proxy-server.conf]',
  path    => '/tmp//_etc_swift_proxy-server.conf/fragments/33_swift_ceilometer',
  replace => 'true',
  require => 'Package[swift]',
}
...
file { '/var/log/ceilometer/swift-proxy-server.log':
  ...
  mode    => '0664',
  owner   => 'swift',
  path    => '/var/log/ceilometer/swift-proxy-server.log',
  require => 'Package[swift]',
}
...
package { 'rsync':
  ...
  before => ['Rsync::Get[/etc/swift/backups/]', 'Rsync::Get[/etc/swift/account.ring.gz]', 'Rsync::Get[/etc/swift/object.ring.gz]', 'Rsync::Get[/etc/swift/container.ring.gz]'],
  name   => 'rsync',
}
...
rsync::get { '/etc/swift/account.ring.gz':
  execuser => 'root',
  name     => '/etc/swift/account.ring.gz',
  options  => '-a',
  path     => '/etc/swift/account.ring.gz',
  source   => 'rsync://10.108.4.2/swift_server/account.ring.gz',
  timeout  => '900',
}

rsync::get { '/etc/swift/backups/':
  execuser  => 'root',
  name      => '/etc/swift/backups/',
  options   => '-a',
  path      => '/etc/swift/backups/',
  recursive => 'true',
  source    => 'rsync://10.108.4.2/swift_server/backups/',
  timeout   => '900',
}

rsync::get { '/etc/swift/container.ring.gz':
  execuser => 'root',
  name     => '/etc/swift/container.ring.gz',
  options  => '-a',
  path     => '/etc/swift/container.ring.gz',
  source   => 'rsync://10.108.4.2/swift_server/container.ring.gz',
  timeout  => '900',
}

rsync::get { '/etc/swift/object.ring.gz':
  execuser => 'root',
  name     => '/etc/swift/object.ring.gz',
  options  => '-a',
  path     => '/etc/swift/object.ring.gz',
  source   => 'rsync://10.108.4.2/swift_server/object.ring.gz',
  timeout  => '900',
}
...
swift::ringsync { 'account':
  name        => 'account',
  notify      => ['Class[Swift::Storage::All]', 'Service[swift-proxy]'],
  ring_server => '10.108.4.2',
}

swift::ringsync { 'container':
  name        => 'container',
  notify      => ['Class[Swift::Storage::All]', 'Service[swift-proxy]'],
  ring_server => '10.108.4.2',
}

swift::ringsync { 'object':
  name        => 'object',
  notify      => ['Class[Swift::Storage::All]', 'Service[swift-proxy]'],
  ring_server => '10.108.4.2',
}
