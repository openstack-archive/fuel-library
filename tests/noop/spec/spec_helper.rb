...
module Noop
  ...
  def self.file_resources_lists_dir
    File.expand_path File.join __FILE__, '..', '..', 'file_resources'
  end

  def self.file_resources_list_file(manifest)
    file_name = manifest.gsub('/', '_').gsub('.pp', '') + '_files.yaml'
    File.join file_resources_lists_dir, file_name
  end

  def self.save_file_resources_list(manifest, data)
    begin
      Dir.mkdir file_resources_lists_dir unless File.directory? file_resources_lists_dir
      file_path = file_resources_list_file manifest
  ...
  def self.package_resources_lists_dir
    File.expand_path File.join __FILE__, '..', '..', 'package_resources'
  end

  def self.package_resources_list_file(manifest)
    file_name = manifest.gsub('/', '_').gsub('.pp', '') + '_packages.yaml'
    File.join package_resources_lists_dir, file_name
  end

  def self.save_package_resources_list(manifest, data)
    begin
      Dir.mkdir package_resources_lists_dir unless File.directory? package_resources_lists_dir
      file_path = package_resources_list_file manifest
      File.open(file_path, 'w') do |list_file|
        YAML.dump(data, list_file)
      end
    rescue
