...
shared_examples 'should_not_install_bin_files_with_puppet' do
  it 'should not install binary files with puppet' do
    binary_files_regexp = %r{^/bin|^/usr/bin|^/usr/local/bin|^/usr/sbin|^/sbin|^/usr/lib|^/usr/share}
    binary_files = []
    downloaded_files = []
...
shared_examples 'OS' do
  ...
  it_behaves_like 'should_not_install_bin_files_with_puppet' if ENV['SPEC_PUPPET_BINARY_FILES'] !~ /false/i
