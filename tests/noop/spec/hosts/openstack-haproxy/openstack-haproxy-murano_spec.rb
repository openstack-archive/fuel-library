...
describe manifest do
  shared_examples 'catalog' do
    ...
    murano_nodes = task.hiera_hash('murano_nodes')

    let(:murano_address_map) do
      task.puppet_function 'get_node_to_ipaddr_map_by_network_role', murano_nodes, 'heat/api'
    end
    ...
    use_murano = task.hiera_structure('murano/enabled', false)
    use_cfapi_murano = task.hiera_structure('murano-cfapi/enabled', false)

    if use_murano and !task.hiera('external_lb', false)
      it "should properly configure murano haproxy based on ssl" do
        public_ssl_murano = task.hiera_structure('public_ssl/services', false)
