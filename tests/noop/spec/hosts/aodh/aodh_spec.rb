...
#      prepare
    ...
    management_vip = Noop.hiera_structure 'management_vip'
    ...
    user = aodh_hash.fetch(:user_name, 'aodh')
    ...
    database_vip = Noop.hiera_structure 'database_vip'
    db = {
      'user_name' => 'aodh',
      'user_password' => 'aodh',
      'database' => 'aodh'
    }
    db['db_hosts'] = database_vip
    if aodh_hash['db']
      db = db.merge(aodh_hash['db'])
    end
    ...
    it 'should properly build connection string' do
      ...
      should contain_aodh_config('database/connection').with(:value => "mysql://#{db['user_name']}:#{db['user_password']}@#{db['db_hosts']}/#{db['database']}#{db_params}")
    end
