...
describe manifest do
  shared_examples 'catalog' do
    ...
    glance_config = Noop.node_hash['glance']
    storage_config = Noop.node_hash['storage']
    ...
    it 'should declare glance classes' do
      should contain_class('glance::api')
    ...
    it 'should configure glance api config' do
      ...
      should contain_glance_api_config('keystone_authtoken/signign_dir').with_value('/tmp/keystone-signing-glance')
    ...
    it 'should configure glance cache config' do
      ...
      if glance_config && glance_config.has_key?('image_cache_max_size')
        ...
        should contain_glance_cache_config('DEFAULT/image_cache_max_time').with_value(glance_image_cache_max_size)
      end
    end
    ...
    it 'should configure glance registry config' do
      ...
      should contain_glance_registry_config('keystone_authtoken/signign_dir').with_value('/tmp/keystone-signing-glance')
    end
    ...
    if storage_config && storage_config.has_key?('images_ceph')
      glance_backend = 'ceph'
      it 'should declare ceph backend' do
        should contain_class('glance::backend::rbd')
      end
    elsif storage_config && storage_config.has_key?('images_vcenter')
      glance_backend = 'vmare'
      it 'should declare vmare backend' do
