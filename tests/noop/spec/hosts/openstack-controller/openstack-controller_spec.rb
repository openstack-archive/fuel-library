  ...
  shared_examples 'catalog' do
    ...
    if primary_controller
      it 'should have explicit ordering between LB classes and particular actions' do
        expect(graph).to ensure_transitional_dependency("Class[nova::api]","Haproxy_backend_status[nova-api]")
        expect(graph).to ensure_transitional_dependency("Haproxy_backend_status[nova-api]",
                                                    "Exec[create-m1.micro-flavor]")
        expect(graph).to ensure_transitional_dependency("Haproxy_backend_status[keystone-public]","Exec[create-m1.micro-flavor]")
        expect(graph).to ensure_transitional_dependency("Haproxy_backend_status[keystone-admin]","Exec[create-m1.micro-flavor]")
        franges = graph.vertices.find_all {|v| v.type == :nova_floating_range }
        if !franges.to_a.empty?
          franges.each do
            |frange|
            expect(graph).to ensure_transitional_dependency("Haproxy_backend_status[nova-api]",frange.ref)
            expect(graph).to ensure_transitional_dependency("Haproxy_backend_status[keystone-public]",frange.ref)
            expect(graph).to ensure_transitional_dependency("Haproxy_backend_status[keystone-admin]",frange.ref)
          end
        end
      end
