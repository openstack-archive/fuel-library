      ...
      context 'with Neutron-ml2-plugin' do
       ...
       if role == 'compute' and enable_dpdk
         it 'should set dpdk-specific options for OVS agent' do
           should contain_neutron_agent_ovs('ovs/datapath_type').with_value('netdev')
           should contain_neutron_agent_ovs('ovs/vhostuser_socket_dir').with_value('/var/run/openvswitch')
         ...
         it 'should skip dpdk-specific options for OVS agent' do
           should contain_neutron_agent_ovs('ovs/datapath_type').with_ensure('absent')
           should contain_neutron_agent_ovs('ovs/vhostuser_socket_dir').with_ensure('absent')
