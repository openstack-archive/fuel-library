...
describe manifest do
  ...
  shared_examples 'catalog' do
    ...
    let(:check_neutron) do
      skip 'Neutron is not used' unless use_neutron
    end

    let(:check_novanet) do
      skip 'Novanet is not used' if use_neutron
    end

    let(:check_neutron_dvr) do
      skip 'No neutron dvr' unless neutron_dvr
    end
    ...
    context 'with Neutron' do
      ...
      it 'should configure neutron L2 population' do
        check_neutron
        skip 'No neutron_l2_pop' unless neutron_l2_pop
        should contain_class('openstack::network').with(
                   'l2_population' => neutron_l2_pop
               )
      end

      context 'segmentation_type = GRE' do
        it 'should configure tunnel_types for neutron and set net_mtu' do
          check_neutron
          skip 'Not GRE' unless segmentation_type == 'gre'
