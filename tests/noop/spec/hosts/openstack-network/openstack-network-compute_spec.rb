...
describe manifest do
  ...
  shared_examples 'catalog' do
    ...
    context 'with Neutron', :if => Noop.hiera('use_neutron') do
      ...
      context 'with neutron_dvr', :if => Noop.hiera_structure('neutron_advanced_configuration/neutron_dvr') do
      ...
      it 'should configure neutron L2 population', :if => Noop.hiera_structure('neutron_advanced_configuration/neutron_l2_pop') do
        should contain_class('openstack::network').with(
                   'l2_population' => neutron_l2_pop
               )
      end

      context 'segmentation_type = GRE', :if => Noop.hiera_structure('neutron_config/L2/segmentation_type') == 'gre'  do
    ...
    context 'with Nova-Network', :unless => Noop.hiera('use_neutron') do
      ...
      context 'segmentation_type = VLAN', :if => Noop.hiera_structure('neutron_config/L2/segmentation_type') == 'vlan' do
