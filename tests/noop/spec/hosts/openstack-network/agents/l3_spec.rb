...
describe manifest do
  shared_examples 'catalog' do
    if (Noop.hiera('use_neutron') and Noop.hiera('role') =~ /controller|compute/)
      ...
      context 'neutron-l3-agent'  do
        it { should contain_class('neutron::agents::l3').with(
          'manage_service' => true
        )}
      end

      context 'neutron-l3-agent on compute', :if => (Noop.hiera('role') == 'compute')  do
      ...
      context 'with Neutron-l3-agent on controller', :if => (Noop.hiera('role') =~ /controller/) do
        na_config = Noop.hiera_hash('neutron_advanced_configuration')
        dvr = na_config.fetch('neutron_dvr', false)
        ...
        if ha_agent
          ...
          it { should_not contain_cluster__neutron__l3 }
        end
      end
    end
  end
