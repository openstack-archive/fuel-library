  ...
  shared_examples 'catalog' do
    ...
    context "when all services have hostnames" do
      ...
      public_services.each do |service|
        ...
        it "should set #{service} resolving for public hostanme" do
          should contain_host("#{public_hostname}_#{service}_public").with(
            'ensure' => 'present',
            'ip'     => public_vip,
          )
        end
      ...
      ia_services.each do |service|
        ...
        it "should set #{service} resolving for internal hostname" do
          should contain_host("#{internal_hostname}_#{service}_internal").with(
            'ensure' => 'present',
            'ip'     => management_vip,
          )
        end
        ...
        it "should set #{service} resolution for admin hostname" do
          should contain_host("#{admin_hostname}_#{service}_admin").with(
            'ensure' => 'present',
            'ip'     => admin_vip,
          )
        end
    ...
    context "when keystone external ip set" do
      ...
      it "should set resolve with keystone public external ip" do
        should contain_host("#{public_hostname}_keystone_public").with(
          'ensure' => 'present',
          'ip'     => public_ip,
        )
      end
      ...
      it "should set resolve with keystone internal external ip" do
        should contain_host("#{internal_hostname}_keystone_internal").with(
          'ensure' => 'present',
          'ip'     => internal_ip,
        )
      end
      ...
      it "should set resolve with keystone admin external ip" do
        should contain_host("#{admin_hostname}_keystone_admin").with(
          'ensure' => 'present',
          'ip'     => admin_ip,
        )
      end
