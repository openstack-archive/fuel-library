...
module PuppetModules
  ...
  ALLOWED_ACTIONS = %w(console list restore compress status update reset install remove)
  ...
  # run the code of the provided block
  # inside the script's root directory
  ...
  # run the code of the provided block
  # inside the puppet modules directory
  # and then return back
  def self.inside_puppet_directory(&block)
    Dir.chdir(dir_path_puppet.to_s, &block)
  end

  # run this command inside the script's root directory
  ...
  # run this command inside the puppet modules directory
  # return true on success
  # @param cmd [String]
  # @return [true,false]
  def self.run_inside_puppet_directory(cmd)
    output "Run: #{cmd}"
    inside_puppet_directory do
      system cmd
    end
  end
  ...
  def self.modules_compress
    ...
    cmd = "timeout #{TIMEOUT} " + cmd if timeout_installed?
    success = run_inside_puppet_directory cmd
    if success
  ...
  def self.prepare_bundler
    ...
    remove_gemfile_lock
    success = run_inside_root 'bundle install'
    error 'Bundler install command failed!' unless success
    success = run_inside_root 'bundle update'
