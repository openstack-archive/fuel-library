      ...
      munge do |value|
        ...
        value = :present if value == :installed
          ...
          if Type::Package.respond_to? :versions and Type::Package.versions.is_a? Hash
            Puppet.debug "Found saved versions structure for package '#{package_name}'"
            versions = Type::Package.versions
          else
            Puppet.debug "Reading versions structure for package '#{package_name}'"
            require 'yaml'
            ...
            Puppet.debug "Saving versions structure for package '#{package_name}'"
            class Type::Package
              def self.versions
                @versions
              end
