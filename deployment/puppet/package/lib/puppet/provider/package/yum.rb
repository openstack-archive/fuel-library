...
Puppet::Type.type(:package).provide :yum, :parent => :rpm, :source => :rpm do
  ...
  def install
    ...
    rollback = false
    rollback = true if diff.is_a? Hash and diff.key? 'installed' and diff.key? 'removed'

    # check if we are updating
    #statuses = [ :purged ,:absent, :held, :latest, :installed, :present ]
    update = false
    #update = true unless statuses.include? from or statuses.include? to
    update = true if from.is_a? String and to.is_a? String
  ...
  def read_diff(file_path)
    ...
    return nil unless diff.is_a? Hash
    diff
  end
