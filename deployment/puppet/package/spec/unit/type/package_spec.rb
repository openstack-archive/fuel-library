...
describe Puppet::Type.type(:package) do
  ...
  describe Puppet::Type.type(:package) do
    ...
    describe Puppet::Type.type(:package), "with package version override" do
      it "takes a version from the file" do
        require 'yaml'
        file = '/etc/versions.yaml'
        file_content = {
            'pkg_with_version' => '1',
        }
        Puppet::Type::Package.versions = nil if Puppet::Type::Package.respond_to? :versions
        File.stubs(:readable?).with(file).returns true
        YAML.stubs(:load_file).with(file).returns file_content
      ...
      it "bypasses if there is no file" do
        file = '/etc/versions.yaml'
        Puppet::Type::Package.versions = nil if Puppet::Type::Package.respond_to? :versions
        File.stubs(:readable?).with(file).returns false
