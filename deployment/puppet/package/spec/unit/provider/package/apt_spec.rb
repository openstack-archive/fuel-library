  ...
  describe "with transactional rollback support" do
    describe "when updating" do
      it "saves diff only when version is changing and there is no rollback file" do
        ...
        rollback_file = '/var/lib/puppet/rollback/asdf_2.0_1.0.yaml'
        @provider.stubs(:read_diff).with(rollback_file).returns nil
      ...
      it "doesn't save diff when package is being removed" do
        @resource.stubs(:[]).with(:ensure).returns :absent
        @provider.stubs(:query).returns({:ensure => '1.0', :status => '1.0', :name => 'asdf', :error => 'ok'})
      ...
      it "saves correct diff to the rollback file" do
        ...
        @provider.stubs(:query).returns({:ensure => '1.0', :status => '1.0', :name => 'asdf', :error => 'ok'})
        file_dir = '/var/lib/puppet/rollback'
        rollback_file = '/var/lib/puppet/rollback/asdf_1.0_2.0.yaml'
        ...
        @provider.stubs(:save_diff).with(rollback_file, diff)
        @provider.properties
        @provider.install
      end
    end
    ...
    describe "when rolling back" do
      ...
      it "calls apt-get with correct package list" do
        ...
        pkgs = [ 'asdf=1.0', 'asdf-lib=1.0', 'asdf-old-dep=1', 'asdf-new-dep-' ]

        @provider.stubs(:read_diff).with(rollback_file).returns(diff)
