module Puppet::Parser::Functions
  ...
  EOS
    ...
    unless nodes.is_a?(Array) && nodes.any? && nodes.first.is_a?(Hash) && nodes.first['uid']
      Puppet.error 'You should provide $nodes_hash as the first argument of this fuction! It should be an array of hashes with node information.'
    end

    unless %w(array string).include? type
      Puppet.error 'Type should be either array or string!'
    end

    unless roles.any?
      Puppet.error 'You should give at list one role to filter!'
    end
