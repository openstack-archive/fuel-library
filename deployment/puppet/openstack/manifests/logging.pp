...
class openstack::logging (
  ...
  if $role == 'client' {
    ...
    ::rsyslog::snippet { '70-zabbix-server':
      content => template("${module_name}/70-zabbix-server.conf.erb"),
    }

    ::rsyslog::snippet { '71-zabbix-agentd':
      content => template("${module_name}/71-zabbix-agentd.conf.erb"),
    }
    ...
    ::rsyslog::snippet { '00-remote.conf':
    ...
    ::rsyslog::snippet { '30-server-remote-log':
