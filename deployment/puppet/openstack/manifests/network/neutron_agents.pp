...
class openstack::network::neutron_agents (
  ...
  if 'dhcp' in $agents {
    ...
    if $enable_tunneling {
      $mgmt_iface = $network_scheme['endpoints']['br-mgmt']['vendor_specific']['phy_interfaces']
      if $network_scheme['interfaces']["$mgmt_iface"]['mtu'] {
        $mtu = $network_scheme['interfaces']["$mgmt_iface"]['mtu'] - 42
      } else {
        $mtu = 1458
      }
    } else {
      $prv_iface = $network_scheme['endpoints']['br-prv']['vendor_specific']['phy_interfaces']
      if $network_scheme['interfaces']["$prv_iface"]['mtu'] {
        $mtu = $network_scheme['interfaces']["$prv_iface"]['mtu']
      } else {
        $mtu = 1500
      }
    }
