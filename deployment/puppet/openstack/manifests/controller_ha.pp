...
class openstack::controller_ha (
   ...
   $nameservers             = undef,
    ...
    if $quantum { class { 'openstack::ha::neutron': } }
    if $queue_provider == 'rabbitmq' { class { 'openstack::ha::rabbitmq': } }
    if $custom_mysql_setup_class == 'galera' { class { 'openstack::ha::mysqld': } }

    if $swift_proxies { class { 'openstack::ha::swift':   balancers => $swift_proxies } }
    if $rgw_balancers { class { 'openstack::ha::radosgw': balancers => $rgw_balancers } }
