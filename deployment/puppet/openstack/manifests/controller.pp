...
class openstack::controller (
  ...
  class { 'openstack::glance':
    ...
    #FIXME(bogdando) glance knows nothing about multiple amqp hosts, hence use the 1st one and consider localhost is also OK
    # Passed amqp_hosts is always rendered as "host:port, host:port" or as "host", so it is safe to use it like this:
    rabbit_host                  => inline_template("<%= @amqp_hosts.split(',')[0].split(':')[0] %>"),
    rabbit_port                  => inline_template("<%= @amqp_hosts.split(',')[0].split(':')[1] %>"),
    rabbit_virtual_host          => $rabbit_virtual_host,
    rabbit_use_ssl               => $rabbit_use_ssl,
    rabbit_notification_exchange => $rabbit_notification_exchange,
    rabbit_notification_topic    => $rabbit_notification_topic,
    #TODO(bogdando) investigate what rabbit_durable_queues should be
    #  see http://lists.openstack.org/pipermail/openstack-dev/2012-December/003673.html
  }
