...
class openstack::controller (
  ...
  class { 'openstack::glance':
    ...
    # Passed amqp_hosts is always rendered as [host:port], so it is safe to use this
    rabbit_host                  => inline_template("<%= @amqp_hosts[0].split(':')[0] %>"),
    rabbit_port                  => inline_template("<%= @amqp_hosts[0].split(':')[1] %>"),
