...
class openstack::keystone (
  ...
  if ($enabled) {
    # Check whether public SSL is enabled
    # if so, this setting must be enabled
    # cause Keystone gives hardcoded URL to
    # clients instead of one from DB and we
    # need to overwrite this behavior
    if $public_ssl {
      keystone_config {
        'DEFAULT/public_endpoint': value => "https://$public_hostname:5000";
      }
    }
