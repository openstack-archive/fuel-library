...
define openstack::ha::haproxy_service (
  ...
  anchor {"openstack-ha-${name}": } ->
  anchor {"openstack-ha-${name}-config-done": } ->
  anchor {"openstack-ha-${name}-done": }

  Anchor["openstack-ha-${name}"] -> Haproxy::Listen[$name]         -> Anchor["openstack-ha-${name}-config-done"]
  Anchor["openstack-ha-${name}"] -> Haproxy::Balancermember[$name] -> Anchor["openstack-ha-${name}-config-done"]

  if $before_start {
    Anchor["openstack-ha-${name}-done"] -> Class['cluster::haproxy_ocf']

  } else {
    Class['cluster::haproxy_ocf'] -> Anchor["openstack-ha-${name}"]
    ...
    exec { "haproxy reload for ${name}":
      ...
      try_sleep   => 1,
      returns     => [0, ''],
      require     => Anchor["openstack-ha-${name}-config-done"],
      before      => Anchor["openstack-ha-${name}-done"],
    }
  }

  if $require_service {
    Service[$require_service] -> Anchor["openstack-ha-${name}"]
  }
}
