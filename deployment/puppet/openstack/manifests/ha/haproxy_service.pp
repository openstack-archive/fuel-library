...
define openstack::ha::haproxy_service (
  ...
  if $public {
    $pkeyaddr = join(any2array($public_virtual_ip), ",")
    $pkey = "$pkeyaddr:$listen_port"
    if $public_ssl {
      $public_bind = { "$pkey" => ['ssl', 'crt', $public_ssl_path] }
    } else {
      $public_bind = { "$pkey" => "" }
    }
  } else {
  ...
  if $internal {
    $ikeyaddr = join(any2array($internal_virtual_ip), ",")
    $ikey = "$ikeyaddr:$listen_port"
    if $internal_ssl {
      $internal_bind = { "$ikey" => ['ssl', 'crt', $internal_ssl_path] }
    } else {
      $internal_bind = { "$ikey" => "" }
    }
  } else {
