...
define openstack::ha::haproxy_service (
  ...
  if $public {
    $key = inline_template("<%= Array(public_virtual_ip) * ',' %>:<%= listen_port %>")
    if $public_ssl {
      $public_bind = { "$key" => ['ssl', 'crt', $public_ssl_path] }
    } else {
      $public_bind = { "$key" => "" }
    }
  } else {
  ...
  if $internal {
    $key = inline_template("<%= Array(internal_virtual_ip) * ',' %>:<%= listen_port %>")
    if $internal_ssl {
      $internal_bind = { "$key" => ['ssl', 'crt', $internal_ssl_path] }
    } else {
      $internal_bind = { "$key" => "" }
    }
  } else {
