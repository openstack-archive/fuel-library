...
class openstack::ha::haproxy (
  ...
  if $swift_proxies {
    $swift_proxies_address_map = get_network_role_to_ipaddr_map($swift_proxies, 'swift/api')
    class { 'openstack::ha::swift':
      server_names => keys($swift_proxies_address_map),
      ipaddresses  => values($swift_proxies_address_map),
    }
  }
