...
def get_authconfig(cfg_file):
    ...
    with open(cfg_file) as f:
        for line in f:
            ...
            if rg:
                rv_key = rg.group(1).strip(stripchars)
                rv[rv_key] = rg.group(2).strip(stripchars)
    return rv
...
class NeutronXxx(object):
    def __init__(self, openrc, retries=20, sleep=2):
        ...
        while True:
            if ret_count <= 0:
                print(">>> Keystone error: no more retries "
                      "for connect to keystone server.")
                sys.exit(1)
            try:
                ...
                if re.search(r"Connection\s+refused$", errmsg, re.I) or \
                   ...
                   re.search(r"'*NoneType'*\s+object\s+has\s+no\s+attribute"
                             r"\s+'*__getitem__'*$", errmsg, re.I) or \
                   re.search(r"No\s+route\s+to\s+host$", errmsg, re.I):
                    print(">>> Can't connect to {0}, wait for server "
                          "ready...".format(self.auth_config['OS_AUTH_URL']))
    ...
    def get_ports(self):
        ...
        while True:
            if ret_count <= 0:
                print(">>> Neutron error: no more retries "
                      "for connect to keystone server.")
                sys.exit(1)
            try:
    ...
    def get_ifnames_for(self, port_owner, activeonly=False,
                        port_id_part_len=11):
        ...
        for i in self.get_ports_by_owner(port_owner, activeonly=activeonly):
            rv.append("{0}{1} {2}".format(
                port_name_prefix, i['id'][:port_id_part_len],
...
if __name__ == '__main__':
    ...
    parser.add_option("-c", "--auth-config", dest="authconf",
                      default="/root/openrc", help="Authenticatin config FILE",
                      metavar="FILE")
    parser.add_option("-r", "--retries", dest="retries", type="int",
                      default=50, help="try NN retries for get keystone token",
                      metavar="NN")
    parser.add_option("-a", "--activeonly", dest="activeonly",
