...
case $production {
  'prod', 'docker': {
    ...
    # Monitord user
    keystone_tenant { 'monitord':
      ensure  => present,
      enabled => 'True',
    }
    keystone_role { 'monitord':
      ensure => present,
    }
    keystone_user { $::fuel_settings['monitord']['user']:
      ensure   => present,
      password => $::fuel_settings['monitord']['password'],
      enabled  => 'True',
      tenant   => 'monitord',
    }
    keystone_user_role { 'monitord@monitord':
      ensure => present,
      roles  => ['monitord'],
    }
