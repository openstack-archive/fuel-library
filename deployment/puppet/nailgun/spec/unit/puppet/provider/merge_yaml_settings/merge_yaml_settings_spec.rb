...
describe provider_class do
  ...
  it 'should merge settings if sample is file and settings is hash' do
    ...
    provider.stubs(:write_to_file).with(dest_filepath, YAML.dump(result)).returns true
    provider.create()
  end
  ...
  it 'should FAIL if file can not be written' do
    ...
    provider.stubs(:get_dict).with(sample_filepath).returns(sample)
    provider.stubs(:get_dict).with(settings).returns(settings)
    provider.stubs(:write_to_file).with(dest_filepath, YAML.dump(result)).returns false
  ...
  it 'should use sample settings if other is not present' do
    ...
    provider = provider_class.new(resource)
    provider.stubs(:write_to_file).with(dest_filepath, YAML.dump(sample)).returns true
    provider.create()
  end
  ...
  it 'should use new settings if other is not present' do
    ...
    provider = provider_class.new(resource)
    provider.stubs(:write_to_file).with(dest_filepath, YAML.dump(settings)).returns true
    provider.create()
  end
