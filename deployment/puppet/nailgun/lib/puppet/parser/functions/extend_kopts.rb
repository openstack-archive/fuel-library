module Puppet::Parser::Functions
  ...
  EOS
    ...
    hash_new_kopts, new_kopts_keys = string_to_hash(args[0])
    hash_default_kopts, default_kopts_keys = string_to_hash(args[1])

    keys = Set.new(new_kopts_keys + default_kopts_keys)

    return hash_to_string(hash_default_kopts.merge(hash_new_kopts), keys)
  end
end
