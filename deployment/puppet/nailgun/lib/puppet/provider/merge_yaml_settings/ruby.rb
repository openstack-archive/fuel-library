...
Puppet::Type.type(:merge_yaml_settings).provide(:ruby) do
    ...
    def create
        ...
        if not (merged_settings.empty?)
          raise Puppet::Error, "merge_yaml_settings: the file #{@resource[:name]} can not be written!" if !write_to_file(@resource[:name], merged_settings.to_yaml)
        end
    end
