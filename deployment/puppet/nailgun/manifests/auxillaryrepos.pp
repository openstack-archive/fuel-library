class nailgun::auxillaryrepos(
  ...
  exec { "create_ubuntu_repo_dirs":
    path     => '/bin:/sbin:/usr/bin:/usr/sbin',
    command => "bash -c \"mkdir -p ${ubuntu_dir}/pool/{main,restricted} ${ubuntu_dir}/dists/auxillary/{main,restricted}/binary-amd64/\"",
    creates => "${ubuntu_dir}/pool",
  }
  exec { "create_ubuntu_repo_Packages":
    path     => '/bin:/sbin:/usr/bin:/usr/sbin',
    command => "bash -c \"touch ${ubuntu_dir}/dists/auxillary/{main,restricted}/binary-amd64/Packages\"",
