  ...
  case $production {
    'prod', 'docker': {
      ...
      cobbler_profile { 'bootstrap':
        ...
        menu      => true,
        kickstart => '',
        kopts     => "boot=live toram components fetch=http://${::fuel_settings['ADMIN_NETWORK']['ipaddress']}:8080/bootstrap/root.squashfs biosdevname=0 url=http://${::fuel_settings['ADMIN_NETWORK']['ipaddress']}:8000/api mco_user=${mco_user} mco_pass=${mco_pass}",
        ksmeta    => '',
        server    => $real_server,
