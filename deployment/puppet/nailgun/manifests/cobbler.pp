  ...
  case $production {
    'prod', 'docker': {
      ...
      cobbler_profile { 'bootstrap':
        ...
        menu      => true,
        kickstart => '',
        kopts     => "intel_pstate=disable console=ttyS0,9600 console=tty0 biosdevname=0 net.ifnames=0 url=${nailgun_api_url} mco_user=${mco_user} mco_pass=${mco_pass}",
        ksmeta    => '',
        server    => $real_server,
