  ...
  case $production {
    'prod', 'docker': {
      ...
      cobbler_profile { 'ubuntu_bootstrap':
        ...
        menu      => true,
        kickstart => '',
        kopts     => extend_kopts("$bootstrap_path/metadata.yaml", "console=ttyS0,9600 console=tty0 panic=60 ethdevice-timeout=${bootstrap_ethdevice_timeout} boot=live toram components fetch=http://${server}:8080/bootstrap/ubuntu/root.squashfs biosdevname=0 url=${nailgun_api_url} mco_user=${mco_user} mco_pass=${mco_pass}"),
        ksmeta    => '',
        server    => $real_server,
