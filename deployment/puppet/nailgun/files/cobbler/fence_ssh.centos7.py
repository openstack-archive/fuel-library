...
def set_power_status(conn, options):
    if options["--action"] == "off":
        try:
            conn.sendline("sh -c '(sleep 1;/sbin/reboot)' &>/dev/null &")
            conn.log_expect(options, options["--command-prompt"],
...
def main():
    ...
    options["ssh_options"] = "{0} {1} {2} '/bin/bash -c \"PS1={3} /bin/bash " \
                             "--noprofile --norc\"'".format(strict,
                             serveralive, no_stdin, options["-c"])
    options["-X"] = "{0} {1} '/bin/bash -c \"PS1={2}  /bin/bash " \
                    "--noprofile --norc\"'".format(strict, serveralive,
                    no_stdin, options["-c"])
