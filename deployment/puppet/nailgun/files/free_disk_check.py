...
KEYSTONE_URL = '{0}/keystone/v2.0/tokens/'.format(SERVER_URL)
...
def api_notification(topic, message):
    ...
    auth = requests.post(
        ...
        data=json.dumps({
            'auth': {
                'passwordCredentials': {
                    'username': API_CONFIG['KEYSTONE_USER'],
                    'password': API_CONFIG['KEYSTONE_PASS'],
                },
                'tenantName': 'admin',
            }
        }),
    ...
    token = auth.json()['access']['token']['id']
