...
class cluster::mysql (
  ...
  exec { 'wait-for-synced-state':
    path        => '/bin:/sbin:/usr/bin:/usr/sbin',
    command     => "mysql ${user_password_string} -Nbe \"show status like 'wsrep_local_state_comment'\" | grep -q Synced && sleep 10",
    try_sleep   => 5,
    tries       => 60,
    refreshonly => true,
  }
  ...
  # TODO(aschultz): make this stuff more idempotent friendly
  Exec['create-init-file'] ->
    Service['mysqld'] ->
      Exec['wait-initial-sync'] ->
        Exec['wait-for-synced-state'] ->
