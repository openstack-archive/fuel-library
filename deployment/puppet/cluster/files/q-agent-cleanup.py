        ...
        def wrapper(*args, **kwargs):
            ...
            while True:
                try:
                    ...
                    if not pattern and not re.search(pattern):
                        raise e
                    i += 1
                    if i >= RETRY_COUNT:
                        raise e
                    LOG.info("retry request {0}: {1}".format(i, e))
                    sleep(RETRY_DELAY)
...
if __name__ == '__main__':
    ...
    options = vars(args)
    RETRY_COUNT = options.get('retries', 50)
    RETRY_DELAY = options.get('sleep', 2)
    ...
    cleaner = NeutronCleaner(options=options, log=LOG)
    rc = 0
