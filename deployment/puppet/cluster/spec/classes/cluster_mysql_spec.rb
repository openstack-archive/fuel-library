  ...
  shared_examples_for 'cluster::mysql configuration' do
    context 'with valid params' do
      ...
      it 'configures a cs_resource' do
        ...
        should contain_cs_resource('p_mysqld').that_comes_before('Service[mysqld]')
      ...
      it 'creates exec to wait for replication' do
        should contain_exec('wait-initial-sync').that_notifies('Exec[wait-for-synced-state]')
        should contain_exec('wait-for-synced-state').that_comes_before('Exec[rm-init-file]')
      end
