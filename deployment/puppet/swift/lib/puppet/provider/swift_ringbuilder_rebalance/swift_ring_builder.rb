...
Puppet::Type.type(:swift_ringbuilder_rebalance).provide(:swift_ring_builder) do
  ...
  def balance=(value)
    ...
    @resource[:tries].times do
      ...
      # Don't raise error if swift-ring-builder see no partitions to reassign
      return true if rebalance_output =~ /No partitions could be reassigned/
      # Don't raise error if swift-ring-builder refuses to save rebalance
      return true if rebalance_output =~ /Cowardly refusing to save rebalance/
      # Exit if balance is equal to the desired one
      return true if self.balance == value
      sleep @resource[:try_sleep]
      try += 1
    end
    # Rebalance failed, raise an error
