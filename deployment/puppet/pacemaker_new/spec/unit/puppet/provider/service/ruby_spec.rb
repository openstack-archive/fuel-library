...
describe Puppet::Type.type(:service).provider(:ruby) do
  ...
  before :each do
    ...
    @class.stubs(:service_location_exists?).returns(true)
  ...
  context '#status' do
    it 'should return the correct status values' do
      @class.stubs(:primitive_is_running?).returns(true)
      @class.unstub(:get_primitive_puppet_status)
      expect(@class.status).to eq :running
      @class.stubs(:primitive_is_running?).returns(false)
    ...
    it 'counts a service as stopped if location constraint is missing' do
      @class.stubs(:get_primitive_puppet_status).returns(:running)
      @class.stubs(:service_location_exists?).returns(false)
      expect(@class.status).to eq :stopped
      @class.stubs(:service_location_exists?).returns(true)
      expect(@class.status).to eq :running
    end
