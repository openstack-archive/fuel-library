class openstack_tasks::glance::glance {
  ...
  case $glance_backend {
    ...
    'vmware': {
      ...
      if $glance_vcenter_insecure {
        $glance_vc_insecure  = true
        $vcenter_ca_filepath = undef
      }
      elsif ! empty($glance_vcenter_ca_content) and ! $glance_vcenter_insecure {
        $vcenter_ca_filename = $glance_vcenter_ca_file['name']
        $vcenter_ca_filepath = "/etc/glance/${vcenter_ca_filename}"
        $glance_vc_insecure  = false
      ...
      class { '::glance::backend::vsphere':
          ...
          vcenter_insecure        => $glance_vc_insecure,
