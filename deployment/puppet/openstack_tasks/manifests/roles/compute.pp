class openstack_tasks::roles::compute {
  ...
  if $allocated_hugepages['2M'] > 0 {
    $use_2m_huge_pages = true
  } else {
    $use_2m_huge_pages = false
  }
  if $allocated_hugepages['1G'] > 0 {
    $use_1g_huge_pages = true
  } else {
    $use_1g_huge_pages = false
  }
  ...
  if $::osfamily == 'Debian' {
    if $use_huge_pages {
      if $use_1g_huge_pages {
        $1g_hugepages_opts_ensure = 'present'
      ...
      $1g_hugepages_opts_ensure = 'absent'
    }
    ...
    file_line { 'libvirt_1g_hugepages_apparmor':
      ...
      ensure  => $1g_hugepages_opts_ensure,
    }
