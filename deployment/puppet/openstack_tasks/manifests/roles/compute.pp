class openstack_tasks::roles::compute {
 ...
 if $repo_type != 'uca' {
  ...
  if $::operatingsystem == 'Ubuntu' {
    ...
    include ::sysfs
    sysfs_config_value { 'CPUgovernor' :
      ensure => 'present',
      name   => '/etc/sysfs.d/cpugovernor.conf',
      value  => $governor,
      sysfs  => '/sys/devices/system/cpu/cpu*/cpufreq/scaling_governor',
      }

    Package<| title == 'cpufrequtils'|> -> Sysfs_config_value<| title == 'CPUgovernor' |> ~>Service<| title == 'cpufrequtils'|>
