class openstack_tasks::openstack_network::server_config {
  ...
  $db_user     = fetch_value($neutron_config, ['database', 'user'], 'neutron')
  $db_name     = fetch_value($neutron_config, ['database', 'name'], 'neutron')
  $db_host     = fetch_value($neutron_config, ['database', 'host'], $database_vip)
  ...
  $l2_population           = fetch_value($neutron_advanced_config, ['neutron_l2_pop'], false)
  ...
  $mechanism_drivers         = delete(fetch_value($neutron_config, ['L2', 'mechanism_drivers'], concat($default_mechanism_drivers,$l2_population_mech_driver,$sriov_mech_driver)), '')
  $flat_networks             = ['*']
  ...
  $_path_mtu = fetch_value($neutron_config, ['L2', 'path_mtu'], undef)
  ...
  if $segmentation_type == 'vlan' {
    ...
    $tunnel_id_ranges  = [fetch_value($neutron_config, ['L2', 'tunnel_id_ranges'])]
    $physical_network_mtus = generate_physnet_mtus($neutron_config, $network_scheme, {
      'do_floating' => $do_floating,
      'do_tenant'   => false,
      'do_provider' => false
