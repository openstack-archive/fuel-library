Puppet::Type.type(:network_config).provide(:interfaces) do
  ...
  def create
    ...
    File.open(@config_file.to_s, 'w') do |f|
      ...
      if @memory_values['onboot']="true" then f.write("auto #{@resource[:name]}\n") end
      case @memory_values['bootproto']
        when /dhcp/
          if_str = "iface #{@resource[:name]} inet dhcp"
        when /static/
          if_str = "iface #{@resource[:name]} inet static"
        when /none/
          if_str = "iface #{@resource[:name]} inet manual"
      end
      if not @resource['name'] == 'lo' then f.write("#{if_str}\n")
      else f.write("iface #{@resource[:name]} inet loopback\n") end
    ...
    system("grep -q \"source.*#{@resource[:name]}\" #{@@main_config_file} || echo \"source #{@config_file}\" >> #{@@main_config_file}")
    system("ifdown #{@resource[:name]};sleep 2;ifup #{@resource[:name]}")
  end

  def destroy
    system("ifdown #{@resource[:name]}")
    system("sed -i /source.*#{@resource[:name]}/d #{@@main_config_file}")
