...
if $use_syslog and !$debug { #syslog and nondebug case
  ...
  File['nova-logging.conf'] ~> Service['nova-api']
  File['nova-logging.conf'] ~> Service['nova-compute']
  File['nova-logging.conf'] ~> Service['libvirt']
  File['nova-logging.conf'] ~> Service['tgtd']
