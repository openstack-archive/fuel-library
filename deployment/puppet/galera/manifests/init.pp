...
class galera (
  ...
  # update innodb_log_file_size only if there is no 'old' wsrep configuration exists yet and it
  # weren't yet defined by puppet
  if !defined(File['/etc/mysql/conf.d/wsrep.cnf']) and !$::galera_conf_exists {
