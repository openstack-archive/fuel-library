...
class keystone_ha(
  $admin_token,
  $ha_mode              = true,
  $package_ensure       = undef,  # undef here, because if we pass
  $bind_host            = undef,  # undef as parameter value,
  $verbose              = undef,  # puppet got default value
  $debug                = undef,  # from class header
  $use_syslog           = undef,
  $syslog_log_facility  = undef,
  $catalog_type         = undef,
  $memcache_servers     = undef,
  $memcache_server_port = undef,
  $enabled              = undef,
