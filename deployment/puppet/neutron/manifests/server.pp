...
class neutron::server (
  ...
  exec {'insert service tenant ID':
    onlyif  => "bash -c \"source /root/openrc ; keystone tenant-list\" | grep \"${nova_admin_tenant_name}\" | awk -F'|'  '{print \$2}' > /tmp/serviceid; head -n1 /tmp/serviceid | grep -xEe '\s*[a-zA-Z0-9]+\s*'",
