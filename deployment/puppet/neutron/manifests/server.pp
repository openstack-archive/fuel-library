...
class neutron::server (
  ...
  Keystone_tenant<| title == "${nova_admin_tenant_name}" |> -> Exec['get_service_tenant_ID']
  Keystone_user_role<| title == "${neutron_config['keystone']['admin_user']}@${nova_admin_tenant_name}" |> ->
      Exec['get_service_tenant_ID']
  Keystone_endpoint<| title == "${neutron_config['keystone']['admin_user']}" |> ->
    Exec['get_service_tenant_ID']
