...
class neutron::server (
  ...
  neutron_config {
    ...
    'DEFAULT/api_workers':          value => $neutron_config['server']['api_workers'] ? {
                                                default => $neutron_config['server']['api_workers'],
                                                undef => min($::processorcount + 0, 50 + 0)
                                    };
    'DEFAULT/rpc_workers':          value => $neutron_config['server']['rpc_workers'] ? {
                                                default => $neutron_config['server']['rpc_workers'],
                                                undef => min($::processorcount + 0, 50 + 0)
                                    };
