...
')
  ...
  $mgmt_vlan = inline_template("<%= begin @network_scheme['transformations'].select{|n| n['action'] == 'add-patch' and n['bridges'][1] == 'br-mgmt'}.first['tags'][0] rescue 'fail' end %>")
  $storage_vlan = inline_template("<%= begin @network_scheme['transformations'].select{|n| n['action'] == 'add-patch' and n['bridges'][1] == 'br-storage'}.first['tags'][0] rescue 'fail' end %>")
  $ext_vlan = inline_template("<%= begin @network_scheme['transformations'].select{|n| n['action'] == 'add-patch' and n['bridges'][1] == 'br-ex'}.first['tags'][0] rescue 'fail' end %>")
  ...
  # Apply modification only if network config is appropriate and it's CentOS
  if (
    $::osfamily == 'RedHat' and
