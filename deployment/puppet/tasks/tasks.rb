...
class Pathname
  ...
  def puppet_environment_name
    "task__#{self.sub_ext('').to_s.gsub('/', '__')}"
  end
end
...
module Tasks
  ...
  def self.dir_path_new_modular
    dir_path_root + Pathname.new('osnailyfacter/new_modular')
  end
  ...
  def self.dir_path_environments
    dir_path_root + Pathname.new('environments')
  end
  ...
  def self.create_new_modular_tasks
    all_modular_tasks.each do |task|
      ...
      path = dir_path_new_modular + task
      path.dirname.mkpath
  ...
  def self.create_environments_structure
    all_modular_tasks.each do |task|
      content = generate_new_modular_task task
      path = dir_path_environments + Pathname.new(task.puppet_environment_name) + Pathname.new('manifests/site.pp')
      path.dirname.mkpath
      puts "Writing a new environment manifest: #{path}"
      File.open(path.to_s, 'w') do |file|
        file.puts content
      end
    end
