...
module Pacemaker
  ...
  def primitive_is_complex?(primitive)
    return nil unless primitive_exists? primitive
  ...
  def primitive_is_clone?(primitive)
    return nil unless primitive_is_complex? primitive
  ...
  def get_cluster_debug_report
    ...
    primitives_status_by_node.each do |primitive, data|
      ...
      report += "* #{primitive_name}:"
      report += ' (FAIL)' if primitive_has_failures? primitive
      report += ' (UNMANAGE)' unless primitive_is_managed? primitive
      report += "\n"
      report += '  ' if data.any?
      report += data.map { |k,v| "#{k}: #{v}" }.join ' | '
      report += "\n"
    end
    report
  end
  ...
  def primitive_has_master_running?(primitive, node = nil)
    return nil unless primitive_is_multistate? primitive
    status = primitive_status primitive, node
    return unless status
    status == 'master'
  end
  ...
  def cleanup_with_wait(primitive)
    ...
    retry_block_until_true do
      cib_reset
      !primitive_is_running?(primitive).nil?
    end
