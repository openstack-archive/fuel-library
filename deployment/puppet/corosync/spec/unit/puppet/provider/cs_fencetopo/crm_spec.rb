 ...
 let(:foo_instance) { foo_provider.class.instances }
  ...
  describe "#create" do
    ...
    it "should create topology singleton with corresponding nodes list and fence primitives" do
      ...
      provider.expects(:exec_withenv).with(' --force configure fencing_topology node-1.test.local: stonith__ipmi_reset__node-1 stonith__psu_off__node-1,stonith__psu_on__node-1 node-2.test.local: stonith__ipmi_reset__node-2 stonith__psu_off__node-2,stonith__psu_on__node-2 2>&1', {})
      provider.create
      provider.flush
    end
