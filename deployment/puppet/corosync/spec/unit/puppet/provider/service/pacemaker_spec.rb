...
describe Puppet::Type.type(:service).provider(:pacemaker) do
  ...
  context '#status' do
    ...
    it 'gets service status either globally or locally if called with node name' do
      ...
      @class.status hostname
      @class.expects(:get_primitive_puppet_status).with name, nil
      @class.status
    end
  ...
  context '#start' do
    ...
    it 'tries to unban the service by the short name' do
      @class.expects(:unban_primitive).with(name)
      @class.start
    end
  ...
  context '#stop' do
    ...
    it 'uses Ban to stop the service and waits for it to stop locally if service is complex' do
      ...
      @class.expects(:ban_primitive).with name
      @class.stop
    end
    ...
    it 'uses Stop to stop the service and waits for it to stop globally if service is simple' do
      ...
      @class.expects(:stop_primitive).with full_name
      @class.stop
    end
  end
  ...
  context '#restart' do
    it 'does not stop or start the service if it is not locally running' do
      @class.stubs(:primitive_running?).with(name, hostname).returns(false)
    ...
    it 'stops and start the service if it is locally running' do
      @class.stubs(:primitive_running?).with(name, hostname).returns(true)
