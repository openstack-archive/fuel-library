...
describe Puppet::Provider::Pacemaker_common do
  ...
  context 'constraints' do
    context 'location' do
      it 'can get the locations structure from the CIB XML' do
        expect(@class.constraint_locations).to be_a(Hash)
        expect(@class.constraint_locations['clone_p_haproxy-on-node-1']).to be_a(Hash)
        expect(@class.constraint_locations['loc_ping_vip__public']['rules']).to be_a(Array)
        expect(@class.constraint_locations['loc_ping_vip__public']['rules'].first['expressions']).to be_a(Array)
        expect(@class.constraint_locations['loc_ping_vip__public']['rules'].first['expressions'].first['id']).to eq('loc_ping_vip__public-expression')
      end
      ...
      it 'can add location constraint' do
        @class.expects(:cibadmin_apply_patch).returns true
        @class.constraint_location_add 'myprimitive', 'mynode', '200'
      end

      it 'can remove location constraint' do
        @class.expects(:cibadmin_remove).returns true
  ...
  context 'xml_generation' do
    ...
    it 'can create a new XML document with specified path' do
...
<rsc_location id='loc_ping_vip__public' rsc='vip__public'>
  <rule boolean-op='or' id='loc_ping_vip__public-rule' score='-INFINITY'>
    ...
    <expression attribute='pingd' id='loc_ping_vip__public-expression-0' operation='lte' value='0'/>
      ...
      it 'can match generated and parsed XML to the original data for a rule based location' do
      ...
      it 'can match generated and parsed XML to the original data for a simple location' do
...
<rsc_colocation id='vip_management-with-haproxy' rsc='vip__management' score='INFINITY' with-rsc='clone_p_haproxy'/>
      ...
      it 'can match generated and parsed XML to the original data for a colocation' do
...
<master id='master_p_rabbitmq-server'>
  ...
  <primitive class='ocf' id='p_rabbitmq-server' provider='mirantis' type='rabbitmq-server'>
    ...
    <operations>
      ...
      it 'can match generated and parsed XML to the original data for a simple primitive' do
      ...
      it 'can match generated and parsed XML to the original data for a complex primitive' do
