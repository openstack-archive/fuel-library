...
eos
  ...
  context 'node status parser' do
    ...
    it 'can determite a global primitive status' do
      ...
      expect(@class.primitive_is_running? 'heat-engine').to be_truthy
      expect(@class.primitive_status 'p_haproxy').to eq('stop')
      expect(@class.primitive_is_running? 'p_haproxy').to be_falsey
    end

    it 'can determine a local primitive status on a node' do
      expect(@class.primitive_status 'heat-engine', 'node-1').to eq('start')
      expect(@class.primitive_is_running? 'heat-engine', 'node-1').to be_truthy
      expect(@class.primitive_status 'p_haproxy', 'node-1').to eq('stop')
      expect(@class.primitive_is_running? 'p_haproxy', 'node-1').to be_falsey
    end
    ...
    it 'can determine that primitive is complex' do
      expect(@class.primitive_is_complex? 'p_haproxy').to be_truthy
      expect(@class.primitive_is_complex? 'p_heat-engine').to be_falsey
    end

    it 'can determine that primitive is multistate' do
      expect(@class.primitive_is_multistate? 'p_haproxy').to be_falsey
      expect(@class.primitive_is_multistate? 'p_heat-engine').to be_falsey
    end

    it 'can determine that primitive is clone' do
      expect(@class.primitive_is_clone? 'p_haproxy').to be_truthy
      expect(@class.primitive_is_clone? 'p_heat-engine').to be_falsey
    end
