...
It works with Open vSwitch interfaces or with standard Linux interfaces. At
the moment, it supports CentOS/RHEL 6.3333nd Ubuntu 12.04+. The module
should have the same behavior across both operating systems.
...
Parameters can be set to customize configuration. For example, if you plan to
use Open vSwitch you can enable it like so:
...
* *bridges* - a bridge is a virtual ethernet L2 switch. You can plug ports into
  a bridge.
* *ports* - a port is an interface that you plug into the bridge. A port may be
  a virtual or a native interface.
...
to many complications while diagnosing issues.

    l23network::l2::bond { 'bond0':
      interfaces      => ['eth1', 'eth2'],
      bridge          => 'br0',  # required for OVS provider
      mtu             => 9000,
      onboot          => true,
...
If **mode** is balance-tcp or lacp, bond-property should be set
to 'active' or 'passive'.

The **xmit_hash_policy** parameter should not be used for *ovs*.
...
The L2::patch resource is a patchcord for connecting two bridges. One
architecture limitation is that two bridges can only be connected by one
patchcord. The name for the patchcord interfaces is calculated automatically
and cannot be changed.

The *ovs* provider can connect OVS-to-OVS and OVS-to-LNX bridges. If you
connect OVS-to-LNX bridges, you should create the OVS bridge first.
...
bridge names. The order of the names depends of the provider implementation.
...
Each L23network resource has interface using puppet 'define' resource.
...
This resource is implemented to manage the interface configuration files. Each
