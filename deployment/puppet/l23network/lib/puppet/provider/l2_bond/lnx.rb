  ...
  def flush
    if ! @property_flush.empty?
        ...
        if @property_flush[:slaves].nil? or @property_flush[:slaves] == :absent
          ...
          if !rm_slave_list.empty?
            ...
            rm_slave_list.each do |slave|
              self.class.set_interface_down(slave)  # need by kernel requirements by design. undocumented :(
          ...
          if !add_slave_list.empty?
            ...
            add_slave_list.each do |slave|
              ...
              self.class.set_interface_down(slave)  # need by kernel requirements by design. undocumented :(
              self.class.set_sys_class("#{bond_prop_dir}/bonding/slaves", "+#{slave}")
              self.class.set_interface_up(slave)
            end
          end
        end
      end
      if @property_flush.has_key? :bond_properties
        ...
        self.class.set_interface_down(@resource[:bond])
        ...
        self.class.set_interface_up(@resource[:bond]) if runtime_bond_state
      ...
      if @property_flush.has_key? :bridge
        ...
        runtime_bond_state  = !self.class.get_iface_state(@resource[:bond]).nil?
        self.class.set_interface_down(@resource[:bond], true)
        ...
        self.class.set_interface_down(@resource[:bond]) if runtime_bond_state
        debug("Change bridge")
      end
      if @property_flush[:onboot]
        self.class.set_interface_up(@resource[:bond])
      end
