  ...
  def flush
    if ! @property_flush.empty?
      ...
      if @property_flush.has_key? :slaves and @old_property_hash.has_key? :slaves
        dpdk_ports = self.class.get_dpdk_ports_mapping
        slaves_to_add = (@property_flush[:slaves] - @old_property_hash[:slaves]).map {|port| dpdk_ports[port]}
        debug("slaves to add #{slaves_to_add}") unless slaves_to_add.empty?
        slaves_to_remove = (@old_property_hash[:slaves] - @property_flush[:slaves]).map {|port| dpdk_ports[port]}
