  ...
  def self.ovs_vsctl_show
    ...
    content.split("\n").each do |line|
      ...
      case line
        ...
        when /^\s+Port\s+"?([\w\-\.]+)\"?$/
          ...
          if _po == _br
            ovs_config[:port][_po][:port_type] << 'bridge' << 'unremovable'
          end
        when /^\s+Interface\s+"?([\w\-\.]+)\"?$/
          ...
          if interface.has_key? _if
            ovs_config[:interface][_if].merge! interface[_if]
          end
    ...
    ovs_config[:port].keys.each do |p_name|
      ...
      if ifaces.size > 1
        ...
        ovs_config[:port][p_name][:port_type] << 'bond' << 'unremovable'
