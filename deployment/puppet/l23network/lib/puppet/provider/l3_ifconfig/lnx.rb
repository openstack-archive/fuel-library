        ...
        if @property_flush[:ipaddr].include?(:absent)
          ...
          if adding_addresses.include? :none
            ...
            adding_addresses.each do |ipaddr|
              ...
              ipaddress = IPAddress::IPv4.new ipaddr
              if old_prefix = changing_prefixes[set_ipaddr]
                ...
                begin
                  self.class.iproute(['addr', 'add', ipaddr, 'broadcast', "#{ipaddress.broadcast}",\
                                      'dev', @resource[:interface], 'label', @resource[:interface]])
                rescue
