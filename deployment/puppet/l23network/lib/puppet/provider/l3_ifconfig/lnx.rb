Puppet::Type.type(:l3_ifconfig).provide(:lnx) do
    ...
    if @property_flush
      ...
      if !@property_flush[:gateway].nil? or !@property_flush[:gateway_metric].nil?
        ...
        if @resource[:gateway] != :absent
          ...
          if !@property_flush[:gateway_metric].nil? and @property_flush[:gateway_metric].to_i >= 0
  ...
  def self.get_if_defroutes_mappings
    ...
    ip_a.each do |line|
      ...
      metric = $4 or 0
      rou_list[$2] = { :gateway => $1, :gateway_metric => metric.to_i } if rou_list[$2].nil?  # do not replace to gateway with highest metric
    end
    return rou_list
  end
