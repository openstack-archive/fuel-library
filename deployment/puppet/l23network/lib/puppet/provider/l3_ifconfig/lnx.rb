Puppet::Type.type(:l3_ifconfig).provide(:lnx) do
  ...
  def interface_has_ip?(ipaddr)
    if_addr_mappings = self.class.get_if_addr_mappings
    interface_ips = if_addr_mappings.fetch(@resource[:interface], {}).fetch(:ipaddr, [])
    interface_ips.include? ipaddr
  end
  ...
  def interface_add_ip(ipaddr)
    begin
      ...
      return if interface_has_ip? ipaddr
      raise exception
    end
  end
  ...
  def interface_del_ip(ipaddr)
    begin
      ...
      return unless interface_has_ip? ipaddr
      raise exception
    end
  end
