...
Puppet::Parser::Functions::newfunction(:generate_network_config, :type => :rvalue, :doc => <<-EOS
    ...
    config_hash[:transformations].each do |t|
      ...
      if (action == :port) and interface and   # does adding phys.interface?
        ...
        splinters_trunks = interface[:L2][:vlan_splinters_trunks]
            ...
            splinters == 'on' or (
              splinters == 'auto' and (
               sp_nics and sp_nics != :undefined and
                sp_nics.split(',').index(trans[:name].to_s) )) or
            splinters_trunks and (
              splinters_trunks == 'auto'))
          trans[:vlan_splinters] = true
        else
          trans[:vlan_splinters] = false
        end
        if trans[:vlan_splinters] and splinters_trunks == 'auto'
          ...
          # zero [0] is needed to pass untagged traffic
          _trunks = [0] + trans[:trunks] + interface[:L2][:trunks]
          _trunks.sort!().uniq!()
