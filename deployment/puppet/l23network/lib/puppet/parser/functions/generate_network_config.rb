...
Puppet::Parser::Functions::newfunction(:generate_network_config, :type => :rvalue, :doc => <<-EOS
    ...
    config_hash[:transformations].each do |t|
      if t[:action] == 'add-port' && t[:name].match(/\.\d+$/)
        ...
        # earlier. For prevent fails in network configuration we should configure
        # main interface as unaddressed interface wich has state is UP
        name = t[:name].split('.')[0]
        ...
        # condition. We should merge properties from this to autocreated early
        # transformation and forget this.
        #
