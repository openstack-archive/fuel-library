...
describe Puppet::Type.type(:l3_ifconfig).provider(:lnx) do
  ...
  it "no existing IPs on the given NIC, just creating" do
    ...
    provider_class.expects(:iproute).with(['addr', 'add', '192.168.1.1/24', 'broadcast', '192.168.1.255',\
                                           'dev', 'eth1', 'label', 'eth1'])
    ...
    provider_class.expects(:iproute).with(['addr', 'add', '192.168.2.2/25', 'broadcast', '192.168.2.127',
                                           'dev', 'eth1', 'label', 'eth1'])
    ...
    provider_class.expects(:iproute).with(['addr', 'add', '192.168.3.3/26', 'broadcast', '192.168.3.63',\
                                           'dev', 'eth1', 'label', 'eth1'])
  ...
  it "Change netmask(increase and decrese) of two IPs and add one IP on the same NIC" do
    ...
    provider_class.expects(:iproute).with(['addr', 'add', '192.168.1.1/24', 'broadcast', '192.168.1.255',\
                                           'dev', 'eth1', 'label', 'eth1']).returns(true)
