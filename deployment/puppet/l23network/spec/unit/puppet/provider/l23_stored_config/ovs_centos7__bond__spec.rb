...
describe Puppet::Type.type(:l23_stored_config).provider(:ovs_centos7) do
  ...
  def fixture_path
    File.join(PROJECT_ROOT, 'spec', 'fixtures', 'provider', 'l23_stored_config', 'ovs_centos7_bond')
  end
  ...
  context "OVS bond with two interfaces" do
    ...
    context 'format file' do
      subject { providers[:ovs_bond_lacp] }
      let(:cfg_file) { subject.class.format_file('filepath', [subject]) }
      it { expect(cfg_file).to match(%r{DEVICE=ovs_bond_lacp}) }
      ...
      it { expect(cfg_file).to match(%r{OVS_OPTIONS="bond_mode=balance-tcp other_config:bond-miimon-interval=50 lacp=active \
other_config:lacp-time=fast bond_updelay=111 bond_downdelay=222"}) }
    ...
    context "parse data from fixture" do
      let(:res) { subject.class.parse_file('ovs_bond_lacp', fixture_data('ifcfg-ovs_bond_lacp'))[0] }
#"BOOTPROTO=none", "DEVICE=ovs_bond_lacp", "ONBOOT=yes", "MTU=9000", "TYPE=OVSBond", "OVS_BRIDGE=br0", "OVS_OPTIONS=\"bond_mode=balance-tcp other_config:bond-miimon-interval=50 lacp=active other_config:lacp-time=fast\"", "DEVICETYPE=ovs", "BOND_IFACES=\"eth2 eth3\"
