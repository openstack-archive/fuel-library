...
describe Puppet::Type.type(:l23_stored_config).provider(:sriov_ubuntu) do
  ...
  context "formating config files" do
    ...
    context 'SRIOV port p2p4' do
      ...
      it { expect(cfg_file).to match(/allow-sriov\s+p2p4/) }
      it { expect(cfg_file).to match(/iface\s+p2p4\s+inet\s+manual/) }
      it { expect(cfg_file).to match(/up\s+echo\s+7\s+>\s*\/sys\/class\/net\/p2p4\/device\/sriov_numvfs/) }
  ...
  context "parsing config files" do
    ...
    context 'SRIOV port p2p4' do
      ...
      it { expect(res[:if_provider].to_s).to eq 'sriov' }
      it { expect(res[:vendor_specific][:sriov_numvfs]).to eq 7 }
    end
  end
