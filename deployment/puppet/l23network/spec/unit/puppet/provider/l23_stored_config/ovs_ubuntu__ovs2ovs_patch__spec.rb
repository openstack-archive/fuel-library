...
describe Puppet::Type.type(:l23_stored_config).provider(:ovs_ubuntu) do
  ...
  context "when formating config files" do
    ...
    context 'for ovs2ovs patchcord p_33470efd-0' do
      ...
      it { expect(cfg_file).to match(/ovs_extra\s+set\s+Interface\s+p_33470efd-0\s+type=patch\s+options:peer=p_33470efd-1/) }
    ...
    context 'for ovs2ovs patchcord p_33470efd-1' do
      ...
      it { expect(cfg_file).to match(/ovs_extra\s+set\s+Interface\s+p_33470efd-1\s+type=patch\s+options:peer=p_33470efd-0/) }
