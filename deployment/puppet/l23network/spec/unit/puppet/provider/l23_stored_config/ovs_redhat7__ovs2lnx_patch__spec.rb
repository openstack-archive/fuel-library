...
describe Puppet::Type.type(:l23_stored_config).provider(:ovs_redhat7) do
  ...
  before(:each) do
    ...
    subject.class.stubs(:provider_of).with('ovs-br').returns('ovs_redhat7')
    subject.class.stubs(:provider_of).with('lnx-br').returns('lnx_redhat7')
    subject.class.stubs(:provider_of).with('ovs-br1').returns('ovs_redhat7')
    subject.class.stubs(:provider_of).with('ovs-br2').returns('ovs_redhat7')
  end
  ...
  context "ovs2lnx patch" do
    ...
    context "parse patch data from fixture" do
      ...
      it { expect(res[:provider].to_s).to eq 'ovs_redhat7' }
    end
