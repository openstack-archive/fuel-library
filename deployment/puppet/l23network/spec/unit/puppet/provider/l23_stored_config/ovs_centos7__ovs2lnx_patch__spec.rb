...
describe Puppet::Type.type(:l23_stored_config).provider(:ovs_centos7) do
  ...
  before(:each) do
    ...
    subject.class.stubs(:provider_of).with('ovs-br').returns('ovs_tt')
    subject.class.stubs(:provider_of).with('lnx-br').returns('lnx_tt')
    subject.class.stubs(:provider_of).with('ovs-br1').returns('ovs_tt')
    subject.class.stubs(:provider_of).with('ovs-br2').returns('ovs_tt')
  end
