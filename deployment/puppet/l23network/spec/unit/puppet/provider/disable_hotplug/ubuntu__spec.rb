...
describe provider_class do
  ...
  it 'Disable hotplug' do
    ...
    FileUtils.stubs(:touch).with('/etc/network/disable-network-interface-hotplug').returns(0)
    provider.create
  end
  ...
  it 'File create error' do
    File.stubs(:exist?).with('/etc/network/disable-network-interface-hotplug').returns(false)
    provider.class.stubs(:udevadm).with('control', '--stop-exec-queue').returns(0)
    FileUtils.stubs(:touch).with('/etc/network/disable-network-interface-hotplug').raises(Puppet::ExecutionFailure,'')
