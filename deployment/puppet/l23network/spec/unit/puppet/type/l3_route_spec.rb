...
describe Puppet::Type.type(:l3_route) do
  ...
  context 'can remove unmanaged routes with the same destination' do
    let(:instances_data) do
      [
          { :name => 'default_route', :destination => '0.0.0.0', :gateway => '1.2.3.4', :metric => '0' },
          { :name => 'another_default_route', :destination => '0.0.0.0', :gateway => '5.6.7.8', :metric => '10' },
    ...
    it 'can get discovered routes' do
      expect(subject.discovered_routes.length).to eq 2
    end
    ...
    it 'can generate a list of routes to remove' do
      ...
      expect(to_remove.length).to eq 1
      expect(to_remove.first[:gateway]).to eq '5.6.7.8'
      expect(to_remove.first[:metric]).to eq '10'
      expect(to_remove.first[:ensure]).to eq :absent
    end
