...
describe 'l23network::l3::ifconfig', :type => :define do
  ...
  it 'Should contain interface_file /etc/network/interfaces.d/ifcfg-eth4' do
    ...
    should contain_file("#{interface_file}").with_content(/address\s+1.1.1.1/)
  end

  it 'Should contain main ipaddr' do
    should contain_file("#{interface_file}").with_content(/address\s+1.1.1.1/)
    should contain_file("#{interface_file}").with_content(/netmask\s+255.255.0.0/)
  end

  it 'Should contains post-up directives for apply IP aliases' do
    params[:ipaddr][1..-1].each do |addr|
      should contain_file("#{interface_file}").with_content(/post-up\s+ip\s+addr\s+add\s+#{addr}\s+dev\s+#{title}/)
      should contain_file("#{interface_file}").with_content(/pre-down\s+ip\s+addr\s+del\s+#{addr}\s+dev\s+#{title}/)
    end
  end
end
