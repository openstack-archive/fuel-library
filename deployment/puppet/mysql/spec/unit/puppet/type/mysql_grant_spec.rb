require 'puppet'
require 'puppet/type/database_grant'
describe Puppet::Type.type(:database_grant) do

  before :each do
    @user = Puppet::Type.type(:database_grant).new(:name => 'foo@localhost/*.*', :privileges => ['ALL', 'PROXY'], :table => ['*.*','@'], :user => 'foo@localhost')
  end
  ...
  it 'should require a name' do
    expect {
      Puppet::Type.type(:database_grant).new({})
    }.to raise_error(Puppet::Error, 'Title or name must be provided')
  end

  it 'should require the name to match the user and table' do
    expect {
      Puppet::Type.type(:database_grant).new(:name => 'foo', :privileges => ['ALL', 'PROXY'], :table => ['*.*','@'], :user => 'foo@localhost')
  ...
  describe 'it should munge privileges' do
    ...
    it 'to just ALL' do
      @user = Puppet::Type.type(:database_grant).new(
