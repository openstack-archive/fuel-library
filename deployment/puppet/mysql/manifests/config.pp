...
class mysql::config(
  ...
  $low_priority_updates=$::mysql::params::low_priority_updates
  ...
  # update innodb_log_file_size only if there is no 'old' mysql configuration exists yet and it
  # weren't yet defined by puppet
  if !defined(File[$config_file]) and !$::mysql_conf_exists {
