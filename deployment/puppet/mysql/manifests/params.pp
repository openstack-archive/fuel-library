...
class mysql::params {
  ...
  $buffer_size             =  inline_template("<%= [($::memorysize_mb * 0.3 + 0).floor, 10000].min %>")
  $mysql_buffer_pool_size  =  inline_template("<%= $buffer_size %>M")
  $mysql_log_file_size     =  inline_template("<%= [($buffer_size * 0.25 + 0).floor, 2047].min %>M")
  $wait_timeout            = 30
