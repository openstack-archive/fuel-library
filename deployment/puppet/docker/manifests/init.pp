class docker (
  ...
  define docker_build ($container = $title) {
    ...
    if $cnt_index != 0 {
      ...
      Anchor['docker-build-start'] -> Exec["container0build"]
    }

    if $container == $name_last {
      Exec["container${cnt_index}check"] -> Notify['build docker containers notice']
    }
  }

  # This creates several new Exec['containter<N><status>'] resources with proper dependency:
  # Exec['container0build'] -> Exec['container0check'] -> ...
  #  Exec['containter1build'] -> Exec['container1check'] -> ...
  #  Exec['containter<N>build'] -> Exec['container<N>check'] -> Notify['build docker containers notice']
