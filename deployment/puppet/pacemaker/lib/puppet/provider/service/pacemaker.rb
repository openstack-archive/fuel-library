...
Puppet::Type.type(:service).provide :pacemaker, :parent => Puppet::Provider::Pacemaker_common do
  ...
  def status
    ...
    cib_reset
    if pacemaker_options[:cleanup_on_status] and (not pacemaker_options[:cleanup_only_if_failures] or primitive_has_failures? name, hostname)
      cleanup_with_wait name, hostname
    end
  ...
  def start
    ...
    if pacemaker_options[:cleanup_on_start] and (not pacemaker_options[:cleanup_only_if_failures] or primitive_has_failures? name, hostname)
      cleanup_with_wait name, hostname
    end
  ...
  def stop
    ...
    enable unless primitive_is_managed? name
    if pacemaker_options[:cleanup_on_stop] and (not pacemaker_options[:cleanup_only_if_failures] or primitive_has_failures? name, hostname)
      cleanup_with_wait name, hostname
    end
