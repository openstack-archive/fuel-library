...
Puppet::Type.type(:pcmk_nodes).provide(:ruby, :parent => Puppet::Provider::Pacemaker_common) do
  ...
  def remove_pacemaker_node_record(node_name)
    cibadmin '--delete', '--obj_type', 'nodes', '--crm_xml', "<node uname='#{node_name}'/>"
  end

  def remove_pacemaker_node_state(node_name)
    cibadmin '--delete', '--obj_type', 'status', '--crm_xml', "<node_state uname='#{node_name}'/>"
  end

  def remove_location_constraint(constraint_id)
    cibadmin '--delete', '--xml-text',  "<rsc_location id='#{constraint_id}'/>"
  end
  ...
  def remove_corosync_node_record(node_number)
    cmapctl '-D', "nodelist.node.#{node_number}"
  end
  ...
  def add_corosync_node_record(node_number=nil, node_addr=nil, node_id=nil, ring_number=0)
    cmapctl '-s', "nodelist.node.#{node_number}.nodeid", 'u32', "#{node_id}"
    cmapctl '-s', "nodelist.node.#{node_number}.ring#{ring_number}_addr", 'str', "#{node_addr}"
  end
