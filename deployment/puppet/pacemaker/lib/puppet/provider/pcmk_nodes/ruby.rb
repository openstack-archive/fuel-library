...
Puppet::Type.type(:pcmk_nodes).provide(:ruby, :parent => Puppet::Provider::Pacemaker_common) do
  ...
  def corosync_nodes_structure
    ...
    cmapctl_nodelist.split("\n").each do |line|
      if line =~ %r(^nodelist\.node\.(\d+)\.nodeid\s+\(u32\)\s+=\s+(\d+))
        ...
        node_name = node_name_bin_id node_id
  ...
  def node_name_bin_id(id)
    pacemaker_nodes_structure.each do |name, node|
      return name if node['id'] == id
    end
...
<diff>
  ...
  def remove_corosync_node(node_name)
    debug "Remove corosync node: '#{node_name}'"
    node_number = corosync_nodes_structure.fetch(node_name, {}).fetch 'number', nil
  ...
  def add_corosync_node(node_name)
    debug "Add corosync node: '#{node_name}'"
