...
Puppet::Type.type(:pcmk_nodes).provide(:ruby, :parent => Puppet::Provider::Pacemaker_common) do
  ...
  def corosync_nodes_structure
    ...
    cmapctl_nodelist.split("\n").each do |line|
      ...
      if line =~ %r(^nodelist\.node\.(\d+)\.ring(\d+)_addr\s+\(str\)\s+=\s+(\S+))
        ...
        node_ip_addr = $3
        nodes[node_number] = {} unless nodes[node_number]
        ring = "ring#{node_ring_number}_addr".to_sym
  ...
  def pacemaker_nodes_states_structure
    return @pacemaker_nodes_states_structure if @pacemaker_nodes_states_structure
    pacemaker_nodes_states_structure = {}
    nodes.each do |uname, node|
      pacemaker_nodes_states_structure[uname] = node['id']
    end
    pacemaker_nodes_states_structure
  end
  ...
  def pacemaker_nodes_states
    debug 'Call: pacemaker_nodes_states'
    pacemaker_nodes_states_structure.keys
  end

  def pacemaker_nodes_states=(expected_nodes)
    debug "Call: pacemaker_nodes_states='#{expected_nodes.inspect}'"
    existing_nodes = pacemaker_nodes_states_structure.keys
    # remove unexpected existing nodes
    existing_nodes.each do |node|
      next if expected_nodes.include? node
      remove_pacemaker_node_state node
    end
  end
