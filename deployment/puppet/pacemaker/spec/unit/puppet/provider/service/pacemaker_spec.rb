...
describe Puppet::Type.type(:service).provider(:pacemaker) do
  ...
  context '#start' do
    ...
    it 'should cleanup a primitive only if there are errors' do
      ...
      @class.start
      @class.stubs(:primitive_has_failures?).returns(false)
      @class.unstub(:cleanup_primitive)
      @class.expects(:cleanup_primitive).with(full_name, hostname).never
      @class.start
    end
    ...
    it 'tries to start the service by its name and full_name' do
      @class.expects(:start_primitive).with(name)
      @class.expects(:start_primitive).with(full_name)
      @class.start
    end

    it 'adds a location constraint for the service by its full name' do
