define ceph::osds::osd (
  $use_prepared_devices # This is boolean parameter that tells the manifest
                        # that the device assumed to be prepared. If set to
                        # true, 'ceph-deploy prepare' will be skipped for the
                        # device.
) {
  ...
  unless $use_prepared_devices {
    exec { "ceph-deploy osd prepare ${deploy_device_name}":
      ...
      try_sleep => 1,
      logoutput => true,
      unless    => "ceph-disk list | grep -q '${name} .*ceph data, active'",
  ...
  exec { "ceph-deploy osd activate ${deploy_device_name}":
    ...
    logoutput => true,
    timeout   => 0,
    onlyif    => "ceph-disk list | grep -q '${name} .*ceph data, prepared'",
  }
