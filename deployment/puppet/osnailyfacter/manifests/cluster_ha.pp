class osnailyfacter::cluster_ha {
  ...
  $multi_host              = true
  $neutron_netnode_on_cnt  = true
  $mirror_type = 'external'
  Exec { logoutput => true }

  class compact_controller (
    $primary_controller,
    $neutron_network_node = $neutron_netnode_on_cnt
  ) {
    ...
    class { 'openstack::controller_ha':
      ...
      neutron_db_password            => $::osnailyfacter::cluster_ha::neutron_config['database']['password'],
      neutron_user_password          => $::osnailyfacter::cluster_ha::neutron_config['keystone']['admin_password'],
      neutron_meta_proxy_secret      => $::osnailyfacter::cluster_ha::neutron_config['metadata']['metadata_proxy_shared_secret'],
      neutron_ha_agents              => $primary_controller ? {true => 'primary', defalt => 'slave'},

      neutron_network_node           => $::use_neutron,
      neutron_netnode_on_cnt         => $::use_neutron,
      cinder                         => true,
