class osnailyfacter::cluster_ha {
  ...
  $amqp_all_nodes = inline_template("<%= @controller_nodes.clone %>")
  if $::internal_address in $amqp_all_nodes {
    # prefer local MQ broker if it exists on this node
    $amqp_external_nodes = delete($amqp_all_nodes, $::internal_all_address)
    $amqp_nodes = concat(['127.0.0.1'], fqdn_rotate($amqp_external_nodes, size($amqp_external_nodes)))
  } else {
    $amqp_nodes = fqdn_rotate($amqp_all_nodes, size($amqp_all_nodes))
  }
