class osnailyfacter::globals::globals {
  ...
  if hiera('amqp_hosts', false) {
    ...
    $amqp_tagged_nodes = get_nodes_hash_by_roles($network_metadata, ['primary-rabbitmq', 'rabbitmq'])
  ...
  $neutron_tagged_nodes = get_nodes_hash_by_tags($network_metadata, ['primary-modular-neutron', 'modular-neutron'])
  ...
  $keystone_tagged_nodes = get_nodes_hash_by_roles($network_metadata, ['primary-keystone', 'keystone'])
  ...
  $database_tagged_nodes = get_nodes_hash_by_roles($network_metadata, ['primary-database', 'database'])
