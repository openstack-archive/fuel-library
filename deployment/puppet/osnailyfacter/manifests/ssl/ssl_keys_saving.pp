class osnailyfacter::ssl::ssl_keys_saving {
  ...
  $pub_certificate_content = fetch_value($public_ssl_hash, ['cert_data', 'content'], '')
  ...
  define cert_file (
    ...
    $public_service = fetch_value($ssl_hash, ["${service}_public"], false)
    $public_usercert = fetch_value($ssl_hash, ["${service}_public_usercert"], false)
    $public_certdata = fetch_value($ssl_hash, ["${service}_public_certdata", 'content'], '')
    $internal_service = fetch_value($ssl_hash, ["${service}_internal"], false)
    $internal_usercert = fetch_value($ssl_hash, ["${service}_internal_usercert"], false)
    $internal_certdata = fetch_value($ssl_hash, ["${service}_internal_certdata", 'content'], '')
    $admin_service = fetch_value($ssl_hash, ["${service}_admin"], false)
    $admin_usercert = fetch_value($ssl_hash, ["${service}_admin_usercert"], false)
    $admin_certdata = fetch_value($ssl_hash, ["${service}_admin_certdata", 'content'], '')
