# This class is used to provide custom network setup with linux bons
# and linux vlans.
#
# ==Parameters
#
...
')
  ...
  $mgmt_vlan = inline_template("<%= @network_scheme['transformations'].select{|n| n['action'] == 'add-patch' and n['bridges'][1] == 'br-mgmt'}.first['tags'][0] %>")
  $storage_vlan = inline_template("<%= @network_scheme['transformations'].select{|n| n['action'] == 'add-patch' and n['bridges'][1] == 'br-storage'}.first['tags'][0] %>")
  $ext_vlan = inline_template("<%= @network_scheme['transformations'].select{|n| n['action'] == 'add-patch' and n['bridges'][1] == 'br-ex'}.first['tags'][0] %>")

  $cidr_br_ex = $network_scheme['endpoints']['br-ex']['IP'][0]
  $ext_address = inline_template("<%= @cidr_br_ex.split('/').first %>")
  $ext_netmask = inline_template("<%= IPAddr.new('255.255.255.255').mask(@cidr_br_ex.split('/').last).to_s %>")
  ...
  validate_string($storage_vlan)
  validate_string($mgmt_vlan)
  validate_string($ext_vlan)
  ...
  file {'/root/ifcfg/convert_to_linux_bonds.sh':
    ...
    content => template("osnailyfacter/${convert_script_name}.erb"),
    mode    => '0755',
    require => File['/root/ifcfg'],
  }
  ifcfg_file {"bond0":
