...
describe Puppet::Type.type(:hiera_config).provider(:ruby) do
  ...
  before(:each) do
    allow(YAML).to receive(:load_file).with('/etc/hiera.yaml').and_return(config_file_data)
    allow(Dir).to receive(:entries).with(override_dir).and_return %w(a.yaml c.yaml b.yaml . .. 1.txt test)
  end
  ...
  context '#retreive' do
    ...
    it 'returns an empty hash if file was not read' do
      expect(YAML).to receive(:load_file).with('/etc/hiera.yaml').and_raise(Errno::ENOENT)
