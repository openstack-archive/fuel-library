...
'
    ...
    it 'should be able to add neutron_network resource' do
      ...
      @compiler.catalog.resource('neutron_network', network_name)['provider_segmentation_id'].should == '5'
  ...
  describe 'Creating neutron resources with fallback segmentation id' do
...
'
    ...
    it 'should be able to add neutron_network resource with fallback segmentation id' do
      @scope.function_create_network([ network_name, network_data, type])
      @compiler.catalog.resource('neutron_network', network_name)['ensure'].should == 'present'
