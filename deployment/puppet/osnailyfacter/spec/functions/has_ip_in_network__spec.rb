...
describe 'has_ip_in_network' do
  ...
  it 'should exist' do
    expect(Puppet::Parser::Functions.function('has_ip_in_network')).to eq 'function_has_ip_in_network'
  end
  ...
  it 'should return true if IP address is from CIDR' do
    ...
    expect(scope.function_has_ip_in_network([ipaddr, cidr])).to eq true
  end

  it 'should return false if IP address is not from CIDR' do
    cidr = '192.168.0.0/255.255.0.0'
    ipaddr = '172.16.0.1'
    expect(scope.function_has_ip_in_network([ipaddr, cidr])).to eq false
  end
