...
describe 'function for formating allocation pools for neutron subnet resource' do
  ...
  describe 'basic tests' do
    ...
    it 'should be able to return with floating nets to mtu map (bond)' do
      expect(@scope.function_generate_physnet_mtus([neutron_config, network_scheme_mtu_on_bond, { 'do_floating' => true, 'do_tenant' => true, 'do_provider' => false }])).to eq(["physnet1:35000", "physnet2:1300"])
    end
    ...
    it 'should be able to return tenant nets to mtu map for multipath from PRV to interfaces, both interfaces has defined MTU' do
      expect(@scope.function_generate_physnet_mtus([neutron_config, network_scheme_multipath_both_mtu, { 'do_floating' => false, 'do_tenant' => true, 'do_provider' => false }])).to eq(["physnet1:1300"])
    end
    ...
    it 'should be able to return tenant nets to mtu map for multipath from PRV to interfaces, one interfaces has defined MTU' do
      expect(@scope.function_generate_physnet_mtus([neutron_config, network_scheme_multipath_one_mtu, { 'do_floating' => false, 'do_tenant' => true, 'do_provider' => false }])).to eq(["physnet1:1300"])
    end
    ...
    it 'should be able to return tenant nets to mtu map for multipath from PRV to interfaces, patchcord defined MTU' do
      expect(@scope.function_generate_physnet_mtus([neutron_config, network_scheme_multipath_mtu_on_path, { 'do_floating' => false, 'do_tenant' => true, 'do_provider' => false }])).to eq(["physnet1:1200"])
    end
