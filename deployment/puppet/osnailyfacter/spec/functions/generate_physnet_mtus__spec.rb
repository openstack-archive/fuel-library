...
{"transformations"=>
 ...
 "endpoints"=> {}
    ...
    it 'should be able to return floating nets to mtu map' do
      expect(@scope.function_generate_physnet_mtus([neutron_config, network_scheme, { 'do_floating' => true, 'do_tenant' => true, 'do_provider' => false }])).to eq(["physnet1:65000", "physnet2:65000"])
    end

    it 'should be able to return without floating nets to mtu map' do
      expect(@scope.function_generate_physnet_mtus([neutron_config, network_scheme, { 'do_floating' => false, 'do_tenant' => true, 'do_provider' => false }])).to eq(["physnet1:65000"])
    end

    it 'should be able to return with floating nets to mtu map (bond)' do
      expect(@scope.function_generate_physnet_mtus([neutron_config, network_scheme_mtu_on_bond, { 'do_floating' => true, 'do_tenant' => true, 'do_provider' => false }])).to eq(["physnet1:1300", "physnet2:35000"])
    end

    it 'should be able to return without floating nets to mtu map (bond)' do
      expect(@scope.function_generate_physnet_mtus([neutron_config, network_scheme_mtu_on_bond, { 'do_floating' => false, 'do_tenant' => true, 'do_provider' => false }])).to eq(["physnet1:1300"])
    end
    ...
    it 'should be able to return with floating nets to mtu map (just OVS port)' do
      expect(@scope.function_generate_physnet_mtus([neutron_config, network_scheme_just_port, { 'do_floating' => true, 'do_tenant' => true, 'do_provider' => false }])).to eq(["physnet1:2000", "physnet2:65000"])
    end
