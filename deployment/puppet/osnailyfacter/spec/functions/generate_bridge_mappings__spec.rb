   ...
   "br-ex"=>{"IP"=>["10.109.1.4/24"], "gateway"=>"10.109.1.1"}}
    ...
    let :neutron_config do
       {"default_floating_net"=>"admin_floating_net",
         ...
         "L2"=>{"phys_nets"=>{"physnet2"=>{"bridge"=>"br-prv", "vlan_range"=>"1000:1030"}},
    ...
    it 'should be able to return floating nets to bridge map' do
      expect(@scope.function_generate_bridge_mappings([neutron_config, network_scheme, { :do_floating => true, :do_tenant   => true, :do_provider => false }])).to eq({'admin_floating_net'=>'br-prv'} )
    end
