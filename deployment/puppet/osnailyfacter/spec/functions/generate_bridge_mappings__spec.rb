  ...
  {"br-fw-admin"=>{"IP"=>["10.109.0.4/24"]},
   ...
   "br-ex"=>{"IP"=>["10.109.1.4/24"], "gateway"=>"10.109.1.1"}}
    ...
    let :neutron_config do
       {"default_floating_net"=>"admin_floating_net",
         ...
         "L2"=>{"phys_nets"=>{"physnet1"=>{"bridge"=>"br-floating", "vlan_range"=>"1000:1030"},
                              "physnet2"=>{"bridge"=>"br-prv", "vlan_range"=>"1000:1030"}},
    ...
    it 'should be able to return floating nets to bridge map' do
      expect(@scope.function_generate_bridge_mappings([neutron_config, network_scheme, { 'do_floating' => true, 'do_tenant' => true, 'do_provider' => false }])).to eq(["physnet1:br-floating", "physnet2:br-prv"])
    end

    it 'should be able to return without floating nets to bridge map' do
      expect(@scope.function_generate_bridge_mappings([neutron_config, network_scheme, { 'do_floating' => false, 'do_tenant' => true, 'do_provider' => false }])).to eq(["physnet1:br-floating"])
    end
