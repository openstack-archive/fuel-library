...
def wait_for_ready_ceph
  59.times.each do |retries|
    sleep 60 if retries > 0
    return if ready_ceph[:exit_code] == 0
  end
  warn 'Ceph is not ready yet, glance images could be not uploaded correctly!
Please check glance image statuses and do it manually if needed.'
end
