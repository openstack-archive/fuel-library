...
function generate_open_ssl_keys {
  for i in $open_ssl_keys
    do
      ...
      local crt_path=$dir_path/$i.crt
      mkdir -p $dir_path
      if [ ! -f $key_path ]; then
        openssl req -newkey rsa:2048 -nodes -keyout $key_path -x509 -days 3650 -out $crt_path -config $CONF_PATH/openssl.cnf 2>&1
        cat $crt_path $key_path > $dir_path/$i.pem
      else
