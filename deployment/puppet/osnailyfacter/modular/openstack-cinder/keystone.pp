...
if $public_ssl_hash['services'] or try_get_value($ssl_hash, 'cinder_public', false) {
...
if try_get_value($ssl_hash, 'cinder_internal', false) {
  ...
  $internal_address  = try_get_value($ssl_hash, 'cinder_internal_hostname', $management_vip)
} else {
  $internal_protocol = 'http'
  $internal_address  = $management_vip
}
if try_get_value($ssl_hash, 'cinder_admin', false) {
  $admin_protocol = 'https'
  $admin_address  = try_get_value($ssl_hash, 'cinder_admin_hostname', $management_vip)
} else {
  $admin_protocol = 'http'
