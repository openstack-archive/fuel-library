...
if $public_ssl_hash['services'] or try_get_value($use_ssl, 'swift_public', false) {
  $public_address = pick(try_get_value($use_ssl, 'swift_public_hostname', ''), $swift_hash['public_vip'],  $public_ssl_hash['hostname'], $public_vip)
