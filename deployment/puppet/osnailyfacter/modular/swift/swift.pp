...
$swift_proxies_addr_list = values(get_network_role_to_ipaddr_map(hiera_hash('swift_proxies', {}), 'swift/api'))
# todo(sv) replace 'management' to mgmt/memcache
$memcaches_addr_list     = values(get_network_role_to_ipaddr_map(hiera_hash('swift_proxy_caches', {}), 'management'))
...
if !($storage_hash['images_ceph'] and $storage_hash['objects_ceph']) and !$storage_hash['images_vcenter'] {
  ...
  if ($deploy_swift_proxy){
    ...
    class { 'openstack::swift::proxy':
      ...
      swift_api_local_ipaddr  => $swift_api_ipaddr,
      swift_storage_local_ipaddr => $swift_storage_ipaddr,
      master_swift_proxy_ip   => $master_swift_proxy_ip,
