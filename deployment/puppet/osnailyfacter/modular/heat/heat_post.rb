...
class HeatPostTest < Test::Unit::TestCase
  def self.create_tests
    ...
    def test_heat_trusts_present
      assert TestCommon::Config.value?('/etc/heat/heat.conf', 'deferred_auth_method', 'trusts')
      assert TestCommon::Config.value?('/etc/heat/heat.conf', 'trusts_delegated_roles', '')
    end

    def test_heat_domain_present
      password = TestCommon::Settings.heat['user_password']
      assert TestCommon::Config.value?('/etc/heat/heat.conf', 'stack_domain_admin', 'heat_admin')
      assert TestCommon::Config.value?('/etc/heat/heat.conf', 'stack_domain_admin_password', password)
    end
