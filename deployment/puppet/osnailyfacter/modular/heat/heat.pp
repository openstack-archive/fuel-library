...
if $public_ssl_hash['services'] or try_get_value($ssl_hash, 'keystone_public', false) {
  $public_auth_protocol = 'https'
  $public_auth_address = pick(try_get_value($ssl_hash, 'keystone_public_hostname', false), $public_ssl_hash['hostname'], $public_vip)
} else {
...
if try_get_value($ssl_hash, 'keystone_internal', false) {
  ...
  $internal_auth_address  = pick($ssl_hash['keystone_internal_hostname'], hiera('keystone_endpoint', ''), hiera('service_endpoint', ''), $management_vip)
} else {
  $internal_auth_protocol = 'http'
  $internal_auth_address  = $service_endpoint
}
if try_get_value($ssl_hash, 'keystone_admin', false) {
  $admin_auth_protocol = 'https'
  $admin_auth_address  = pick($ssl_hash['keystone_admin_hostname'], hiera('keystone_endpoint', ''), hiera('service_endpoint', ''), $management_vip)
} else {
  $admin_auth_protocol = 'http'
  $admin_auth_address  = $service_endpoint
}
