...
$nova_custom_hash               = hiera('nova_custom_hash', {})
...
$nova_config_hash = {
  'DEFAULT/resume_guests_state_on_host_boot'       => { value => pick(hiera('resume_guests_state_on_host_boot'), 'False') },
  'DEFAULT/use_cow_images'                         => { value => pick(hiera('use_cow_images'), 'True') },
  ...
  'libvirt/libvirt_inject_partition'               => { value => '-1' },
}
