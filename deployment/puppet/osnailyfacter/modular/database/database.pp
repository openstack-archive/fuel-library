...
if $enabled {
  ...
  # TODO: put this in my.cnf and nuke wsrep.cnf?
  ...
  $wsrep_options = {
    'mysqld'                           => {
      ...
      'query_cache_size'               => '0',
      'query_cache_type'               => '0',
      'wsrep_cluster_address'          => "gcomm://${server_list}", #TODO deal with this after bootstrap
  ...
  # NOTE: this changes the root password but the mysql module handles it so
  ...
  # TODO: Figure out if this should be part of the my.cnf or should it stay
  # separate
  ...
  # drop our wsrep config in place prior to bootstraping the cluster
  File[$wsrep_config_file] -> Exec<| name == 'boostrap_galera_cluster' |>
  ...
  class { 'cluster::mysql':
    ...
    mysql_socket       => '/var/run/mysqld/mysqld.sock', # TODO: config this
  }

  # this is our pacemaker integration
  ...
  # this requires the galera::status is working
