...
if $use_neutron {
  ...
  $admin_auth_endpoint       = ssl($ssl_hash, {}, 'keystone', 'admin', 'hostname', [hiera('keystone_endpoint', ''), hiera('service_endpoint', ''), $management_vip])

  $neutron_internal_protocol = ssl($ssl_hash, {}, 'neutron', 'internal', 'protocol', 'http')
  $neutron_internal_endpoint = ssl($ssl_hash, {}, 'neutron', 'internal', 'hostname', [$neutron_endpoint, $management_vip])
