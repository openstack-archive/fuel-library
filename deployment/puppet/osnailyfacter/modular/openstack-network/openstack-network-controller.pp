...
if $use_neutron {
  ...
  # puppet conditionals limitation, we need to use 1 level hash
  $keystone_hash = $neutron_config['keystone']
  $keystone_user = $keystone_hash['admin_user'] ? {
    default => $neutron_config['keystone']['admin_user'],
    undef   => 'neutron',
  }
  $keystone_tenant = $keystone_hash['admin_tenant'] ? {
    default => $neutron_config['keystone']['admin_tenant'],
    undef   => 'services',
  }
  # Neutron DB settings
  ...
  $neutron_db_name = $database_hash['name'] ? {
    default => $neutron_config['database']['name'],
    undef   => 'neutron',
  }
  $neutron_db_host = $database_hash['host'] ? {
    default => $neutron_config['database']['host'],
    undef   => hiera('management_vip'),
  }
