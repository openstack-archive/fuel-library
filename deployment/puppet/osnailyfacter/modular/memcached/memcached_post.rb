...
class MemcachedPostTest < Test::Unit::TestCase
  ...
  def test_memcached_on_internal
    ...
    assert TestCommon::Network.connection?(ip, 11211), 'Can not connect to memcached on internal address!'
  end

  def test_memcached_on_localhost
    ip = '127.0.0.1'
    assert TestCommon::Network.connection?(ip, 11211), 'Can not connect to memcached on localhost!'
  end
