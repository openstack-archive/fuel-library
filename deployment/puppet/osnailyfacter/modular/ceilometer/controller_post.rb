...
class CeilometerControllerPostTest < Test::Unit::TestCase
  ...
  def test_ceilometer_api_public_url_accessible
    ...
    assert TestCommon::Network.url_accessible?(url), "Public Ceilometer API URL '#{url}' is not accessible!"
  end

  def test_ceilometer_api_management_url_accessible
    url = "http://#{TestCommon::Settings.management_vip}:#{PORT}"
    assert TestCommon::Network.url_accessible?(url), "Admin Ceilometer API URL '#{url}' is not accessible!"
  end
