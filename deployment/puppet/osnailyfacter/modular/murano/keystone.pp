...
if $public_ssl['services'] or try_get_value($ssl_hash, 'murano_public', false) {
  ...
  $public_address  = pick(try_get_value($ssl_hash, 'murano_public_hostname', ''), $public_ssl['hostname'])
} else {
...
if try_get_value($ssl_hash, 'murano_internal', false) {
  ...
  $internal_address  = pick(try_get_value($ssl_hash, 'murano_internal_hostname', ''), $management_ip)
} else {
...
if try_get_value($ssl_hash, 'murano_admin', false) {
  ...
  $admin_address  = pick(try_get_value($ssl_hash, 'murano_admin_hostname', ''), $management_ip)
} else {
  $admin_protocol = 'http'
