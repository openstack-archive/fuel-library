...
if $public_ssl['services'] or try_get_value($ssl_hash, 'murano_public', false) {
  ...
  $public_address  = pick(try_get_value($ssl_hash, 'murano_public', false), $public_ssl['hostname'])
} else {
...
if try_get_value($ssl_hash, 'murano_internal', false) {
  ...
  $internal_address  = pick(try_get_value($ssl_hash, 'murano_internal', false), $management_ip)
} else {
