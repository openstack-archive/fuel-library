module Puppet::Parser::Functions
  newfunction(:get_nodes_hash_by_tags, :type => :rvalue, :arity => 2, :doc => <<-EOS
...
EOS
    ...
    raise(Puppet::ParseError, "#{errmsg}: 1st argument should be a hash") unless n_metadata.is_a?(Hash)
    raise(Puppet::ParseError, "#{errmsg}: 1st argument should be a valid network_metadata hash") unless n_metadata.has_key?('nodes')
    raise(Puppet::ParseError, "#{errmsg}: 2nd argument should be an list of node tags") unless tags.is_a?(Array)
