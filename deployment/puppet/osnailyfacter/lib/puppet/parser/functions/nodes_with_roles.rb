...
EOS
    ...
    raise Puppet::ParseError, 'Only or two argument allowed.' if args.size < 1 or args.size > 2
    roles, attr = args
    raise Puppet::ParseError, 'Roles should be provided as array' if ! roles.is_a?(Array)
    raise Puppet::ParseError, 'Attribute name should be provided as array' if ! (attr.is_a?(String) or attr == nil)
