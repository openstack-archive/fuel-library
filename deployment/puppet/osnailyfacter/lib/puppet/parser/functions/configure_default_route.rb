...
EOS
  ...
  network_scheme[:endpoints].each do |endpoint, body|
    change_to_vrouter = true if body[:gateway] and endpoint == fw_admin_int and IPAddr.new(network_scheme[:endpoints][management_int][:IP][0]).include?(IPAddr.new(management_vrouter_vip))
  end
