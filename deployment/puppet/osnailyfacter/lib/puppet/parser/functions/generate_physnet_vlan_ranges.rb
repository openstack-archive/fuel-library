...
EOS
   ...
   raise ArgumentError, ("generate_physnet_vlan_ranges(): wrong number of arguments (#{args.length}; must be 3)") if args.length > 3
   args.each do | arg |
     raise ArgumentError, "generate_physnet_vlan_ranges(): #{arg} is not hash!" if !arg.is_a?(Hash)
   end
   ...
   physnet_bridge_map = {}
    neutron_config['L2']['phys_nets'].each do |k,v|
      next if !v['bridge']
      bridge = v['bridge']
      physnet_bridge_map[k] = v['vlan_range'] if !network_scheme['transformations'].select{ |x| x['name'] == bridge }.empty?
    end
