...
Get hashes with SSL data and returns values based on keys
Parameters:
 ...
 resource_type - type of data needed to be got
...
EOS
    ...
    rval = false
    fail "You should provide hash as a first argument!" if not use_ssl_hash.kind_of?(Hash)
    fail "You should provide hash as a second argument!" if not public_ssl_hash.kind_of?(Hash)
    fail "You should provide 'name' for service!" if service_name.empty?
    fail "You should provide 'type' for service!" if service_type.empty?
    fail "You should provide 'type' for resource!" if resource_type.empty?
