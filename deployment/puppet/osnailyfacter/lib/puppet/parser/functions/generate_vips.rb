...
module Puppet::Parser::Functions
  newfunction(:generate_vips) do |args|
    ...
    vips.each do |name, parameters|
      ...
      if iptables_rules
         iptables_substitute_hash = {'NS_VETH' => ns_veth,
                                     'NS_VIP' => parameters['ipaddr'],
                                     'NS_CIDR' => "#{parameters['ipaddr']}/#{cidr_netmask}" }

         vip['ns_iptables_start_rules'] = iptables_rules['start'].join('; ')
         vip['ns_iptables_stop_rules'] = iptables_rules['start'].join('; ')
         iptables_substitute_hash.each_pair { |k, v| vip['ns_iptables_start_rules'] = vip['ns_iptables_start_rules'].gsub(k, v) }
         iptables_substitute_hash.each_pair { |k, v| vip['ns_iptables_stop_rules'] = vip['ns_iptables_stop_rules'].gsub(k, v) }
      end
