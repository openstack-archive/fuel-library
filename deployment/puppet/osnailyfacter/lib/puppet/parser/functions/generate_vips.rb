  ...
  newfunction(:generate_vips) do |args|
    ...
    vips.each do |name, parameters|
      ...
      if iptables_rules
         ...
         vip['ns_iptables_start_rules'] = iptables_rules['ns_start'].join('; ') % iptables_substitute_hash
         vip['ns_iptables_stop_rules'] = iptables_rules['ns_start'].join('; ') % iptables_substitute_hash
      end
