...
# The affected Puppet types can be filtered using the "types" option where you can list the type
...
Puppet::Type.newtype(:override_resources) do
  ...
  def createource(type, title, parameters = {})
  ...
  def eval_generate
    ...
    data.each do |title, parameters|
      ...
      if resource
        ...
        new_resources << createource(type, title, parameters)
      end
    end
