...
Puppet::Type.newtype(:override_resources) do
  ...
  newparam(:types_create, :array_matching => :all) do
  ...
  newparam(:titles_create, :array_matching => :all) do
...
##########
  ...
  # @return [Array<String>]
  def types_create
    types_create = self[:types_create] || []
    return types_create if types_create.any?
    options.fetch 'types_create', []
  end

  # Get the list of title create exceptions
  # @return [Array<String>]
  def titles_create
    titles_create = self[:titles_create] || []
    return titles_create if titles_create.any?
    options.fetch 'titles_create', []
  end
...
##########
  ...
  def type_create?(type)
    if create?
      not types_create.include? type
    else
      types_create.include? type
    end
  end
  ...
  def title_create?(title)
    if create?
      not titles_create.include? title
    else
      titles_create.include? title
    end
  end
    ...
    configuration.each do |type, resources|
      ...
      resources.each do |title, parameters|
        ...
        if resource_present? type, title
          ...
          if create? or type_create? type or title_create? title
