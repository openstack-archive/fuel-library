...
class {'osnailyfacter::patching_classes': stage => 'first'}
...
if $use_quantum {
  ...
  if (
    ...
    $custom_convert_to_linux_bonds = true
    $mgmt_vip_nic = "bond0.$mgmt_vlan"
  } else {
    $mgmt_vip_nic = get_network_role_property('management', 'interface')
  }
} else {
  $mgmt_vip_nic = $::fuel_settings['management_interface']
}
...
class os_common {
  ...
  if $use_quantum {
      ...
      if $custom_convert_to_linux_bonds {
        ...
        class {'custom_project::packages':}
        # Adding custom hosts
        host {'mon.lmc.prz':
          ip => '172.20.0.9',
        }
        host {'mother1.mgmt.cloud.prz':
          ip => '172.17.0.10',
        }
        # Irqbalance
        package {'irqbalance':
