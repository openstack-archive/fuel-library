    ...
    'RedHat': {
      ...
      if ! defined(File[$cinder_volume_default]){
        file { $cinder_volume_default:
          ...
          content => "OPTIONS='--config-file=${cinder_conf} --config-file=${cinder_volume_conf}'",
        }
      }
    ...
    'Debian': {
      ...
      if ! defined(File[$cinder_volume_default]) {
        file { $cinder_volume_default:
          ...
          content => "CINDER_VOLUME_OPTS='--config-file=${cinder_conf} --config-file=${cinder_volume_conf}'",
        }
      }
      ...
      if ! defined(File[$cinder_volume_vmware_init]) {
        file { $cinder_volume_vmware_init:
          ...
          group   => root,
          mode    => '0644',
          source  => 'puppt://modules/vmware/cinder-volume-init-ubuntu',
        }
      }
      ...
      if ! defined(File[$init_link]) {
        file { $init_link:
          ...
          targe  => "{cinder_volume_vmware_init}"
        }
      }
      ...
      if ! defined(File[$upstart_link]) {
        file { $upstart_link:
          ...
          targe  => '/etc/init.d/cinder-volume'
        }
      }
