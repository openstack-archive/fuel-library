...
)
{
  if ! $ha_mode {
    ...
    vmware::nova_compute_simple { $vsphere_clusters: }
    ...
    File['/etc/nova/nova-compute.conf']->
    File<| title == '/etc/sysconfig/openstack-nova-compute' |>->
    Nova::Generic_service['compute']->
    Vmware::Nova_compute_simple<| |>
  } else {
    nova::generic_service { 'compute':
      enabled        => false,
      package_name   => $::nova::params::compute_package_name,
    ...
    vmware::nova_compute_ha { $vsphere_clusters: }

    Nova::Generic_service['compute']->
    anchor { 'vmware-nova-compute-start': }->
    File['vcenter-nova-compute-ocf']->
    Vmware::Nova_compute_ha<||>->
