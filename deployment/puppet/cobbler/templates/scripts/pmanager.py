...
class PreseedPManager(object):
    ...
    def _blacklist_udev_rules(self):
        self.late(
            ...
            "mv /etc/udev/rules.d/$(basename $rules) /etc/udev/rules.d/$(basename $rules).bak; "
            "touch /etc/udev/rules.d/$(basename $rules); "
            "done")
        self.late("udevadm control --reload-rules")

    def _unblacklist_udev_rules(self):
        self.late(
            "for rules in $(ls -1 /lib/udev/rules.d/*.rules); do "
            "if test -e /etc/udev/rules.d/$(basename $rules).bak; "
            "then mv /etc/udev/rules.d/$(basename $rules).bak /etc/udev/rules.d/$(basename $rules); "
