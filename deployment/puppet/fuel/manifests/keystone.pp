class fuel::keystone (
  ...
  keystone_user_role { "${admin_user}@admin":
    ...
    roles   => ['admin'],
    require => [Keystone_user[$admin_user], Keystone_role['admin']],
  }
  ...
  keystone_user { $monitord_user :
    ...
    tenant   => 'services',
    require  => Keystone_tenant['services'],
  }

  keystone_user_role { "${monitord_user}@services":
    ensure  => present,
    roles   => ['monitoring'],
    require => [Keystone_user[$monitord_user], Keystone_role['monitoring']]
  }
