class fuel::rabbitmq (
  ...
  $management_port            = $::fuel::params::management_port,
  $management_bind_ip_address = $::fuel::params::management_bind_ip_address,
  ...
  exec { 'create-mcollective-directed-exchange':
    ...
    command   => "curl -L -i -u ${mco_user}:${mco_password} -H   \"content-type:application/json\" -XPUT \
      ...
      http://${management_bind_ip_address}:${management_port}/api/exchanges/${actual_mco_vhost}/mcollective_directed",
    logoutput => true,
    require   => [
  ...
  exec { 'create-mcollective-broadcast-exchange':
    command   => "curl -L -i -u ${mco_user}:${mco_password} -H \"content-type:application/json\" -XPUT \
      ...
      http://${management_bind_ip_address}:${management_port}/api/exchanges/${actual_mco_vhost}/mcollective_broadcast",
    logoutput => true,
    require   => [
      Service['rabbitmq-server'],
      Rabbitmq_user_permissions["${mco_user}@${actual_mco_vhost}"]
